...
class WebuiBridge {
	...
	#checkTokenReload() {
		if (!this.#TokenAccepted) {
			if (this.#log) console.log(`Webinix -> Token [0x${this.#token.toString(16).padStart(8, '0')}] not accepted. Reload page...`);
		...
		if (this.#isTextBasedCommand(buffer8[this.#PROTOCOL_CMD])) {
			...
			switch (buffer8[this.#PROTOCOL_CMD]) {
				...
				case this.#CMD_CHECK_TK:
					...
					if (status) {
						if (this.#log) console.log(`Webinix -> CMD -> Token Accepted`);
						...
						if (this.#log) console.log(`Webinix -> CMD -> Token Not Accepted`);
					}
					break;
			}
		} else {
