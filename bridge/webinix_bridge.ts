...
class WebuiBridge {
	...
	#close(reason = 0, value = '') {
		...
		if (reason === this.#HEADER_NAVIGATION) {
			if (this.#log) {
				console.log(
					`Webinix -> Close -> Navigation to [${value}]`
				)
			}
	...
	#start() {
		...
		this.#ws.onclose = (event) => {
			...
			if (this.#closeReason === this.#HEADER_NAVIGATION) {
				if (this.#log) {
					console.log(
						`Webinix -> Connection closed du to Navigation to [${this.#closeValue}]`
					)
				}
			} else {
				if (this.#log) {
					...
					case this.#HEADER_CALL_FUNC:
						{
							...
							if (this.#callPromiseResolve[callId]) {
								if (this.#log) {
									console.log(
										`Webinix -> CMD -> Resolving Response #${callId}...`
									)
								}
					...
					case this.#HEADER_JS:
						{
							...
							if (this.#log)
								console.log(`Webinix -> CMD -> JS [${scriptSanitize}]`)
							...
							if (this.#log && !FunError)
								console.log(`Webinix -> CMD -> JS -> Return Success [${FunReturn}]`)
							if (this.#log && FunError)
								console.log(`Webinix -> CMD -> JS -> Return Error [${FunReturn}]`)
					...
					case this.#HEADER_SEND_RAW:
						{
							...
							if (this.#log)
								console.log(`Webinix -> CMD -> Send Raw ${buffer8.length} bytes to [${functionName}()]`)
	...
	#callPromise(fn: string, value: any) {
		...
		if (this.#log) console.log(`Webinix -> Call [${fn}](${value}) (ID:${this.#callPromiseID[0]})`)
