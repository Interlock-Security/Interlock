...
class WebuiBridge {
	...
	constructor({
		...
		document.addEventListener("mousemove", (e) => {
			// Webinix `-webkit-app-region: drag;` custom implementation
			if (e.buttons !== 1) {
	...
	#clicksListener() {
		document.querySelectorAll<HTMLElement>("[id]").forEach(e => {
			if (this.#AllEvents || ((e.id !== '') && (this.#bindsList.includes(e.id)))) {
				if (e.id && !e.dataset.webinix_click_is_set) {
					e.dataset.webinix_click_is_set = "true";
	...
	async #sendData(packet: Uint8Array) {
		...
		while (this.#sendQueue.length > 0) {
			...
			if (currentPacket.length < this.#MULTI_CHUNK_SIZE) {
				...
				// Pre-packet to let Webinix be ready for multi packet
	...
	#checkToken() {
		if (this.#wsIsConnected()) {
			...
			// this.#addID(packet, 0, this.#PROTOCOL_ID)
			this.#sendData(packet);
			if (this.#log) console.log(`Webinix -> Send Token [0x${this.#token.toString(16).padStart(8, '0')}]`);
		}
	}
	...
	#sendDrag(x: number, y: number) {
		if (this.#wsIsConnected()) {
			if (this.#log) console.log(`Webinix -> Send Drag Event [${x}, ${y}]`);
			const packet = Uint8Array.of(
				// Protocol
				// 0: [SIGNATURE]
				// 1: [TOKEN]
				...
				if (fn.trim()) {
					if (fn !== '__webinix_core_api__') {
						if (typeof (window as any)[fn] === 'undefined') {
							...
							if (this.#log) console.log(`Webinix -> Binding backend function [${fn}]`);
						}
					}
				}
			}
	...
	async callCore(fn: string, ...args: DataTypes[]): Promise<DataTypes> {
		return this.call('__webinix_core_api__', fn, ...args);
	}
	// -- WebSocket ----------------------------
	...
	#wsConnect(): void {
		...
		this.#ws = new WebSocket(`${url}:${this.#port}/_webinix_ws_connect`);
	...
	#wsOnClose = (event: CloseEvent) => {
		if (this.#closeReason === this.#CMD_NAVIGATION) {
			this.#closeReason = 0;
			if (this.#log) console.log(`Webinix -> Connection lost. Navigation to [${this.#closeValue}]`);
			this.#allowNavigation = true;
			globalThis.location.replace(this.#closeValue);
		} else {
			if (this.#wsStayAlive) {
				// Re-connect
				if (this.#log) console.log(`Webinix -> Connection lost (${event.code}). Reconnecting...`);
				this.#freezeUi();
		...
		if (this.#isTextBasedCommand(buffer8[this.#PROTOCOL_CMD])) {
			...
			switch (buffer8[this.#PROTOCOL_CMD]) {
				...
				case this.#CMD_NAVIGATION:
					...
					if (this.#log) console.log(`Webinix -> CMD -> Navigation [${url}]`);
				...
				case this.#CMD_WINDOW_RESIZED:
					...
					if (this.#log) console.log(`Webinix -> CMD -> Window Resized [x: ${x}, y: ${y}, width: ${width}, height: ${height}]`);
					break;
				case this.#CMD_NEW_ID:
					...
					if (this.#log) console.log(`Webinix -> CMD -> New Bind ID [${newElement}]`);
				...
				case this.#CMD_CHECK_TK:
					...
					if (status) {
						if (this.#log) console.log(`Webinix -> CMD -> Token [${tokenHex}] Accepted`);
						...
						if (this.#log) console.log(`Webinix -> CMD -> Token [${tokenHex}] Not Accepted. Reload page...`);
			...
			switch (buffer8[this.#PROTOCOL_CMD]) {
				case this.#CMD_SEND_RAW:
					...
					if (this.#log) console.log(`Webinix -> CMD -> Received Raw ${rawDataSize} bytes for [${functionName}()]`);
	 ...
	 * @example - webinix.setEventCallback((e) => {if(e == webinix.event.CONNECTED){ ... }});
	 */
	 ...
	 * is done by including `webinix.js` virtual file in the HTML.
	 *
