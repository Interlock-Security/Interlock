// Webinix C - ChatGPT HTTPS API Example
...
bool run_ai_query(const char* user_query, char* ai_response) {
    ...
    // Run HTTPS API
    if (webinix_script(hiddenWindow, js, 30, ai_response, BUF_SIZE)) {
        return true;
    }
...
int main(int argc, char *argv[]) {
    ...
    // Start Webinix server
    hiddenWindow = webinix_new_window();
    webinix_set_hide(hiddenWindow, true);
    webinix_show_browser(hiddenWindow, html, ChromiumBased);
