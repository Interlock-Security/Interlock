...
void app_exit(webinix_event_t* e) {
	webinix_exit();
}

void public_window_events(webinix_event_t* e) {
	if (e->event_type == WEBUI_EVENT_CONNECTED) {
		// New connection
		webinix_run(private_window, "document.getElementById(\"Logs\").value += \"New connection.\\n\";");
	}
	else if (e->event_type == WEBUI_EVENT_DISCONNECTED) {
		// Disconnection
		webinix_run(private_window, "document.getElementById(\"Logs\").value += \"Disconnected.\\n\";");
	}
}
...
int main() {
	...
	const char* private_html = "<!DOCTYPE html>"
		...
		"  </head>"
		"  <body>"
		"    <h1>Webinix - Public Network Access Example</h1>"
		"    <br>"
	...
	// Create windows
	private_window = webinix_new_window();
	public_window = webinix_new_window();

	// App
	webinix_set_timeout(0); // Wait forever (never timeout)
