...
function load_lib() {
  ...
  webinix_lib = Deno.dlopen(
    lib_path,
    {
      ...
      webinix_bind_interface: {
        parameters: ['pointer', 'buffer', 'function'],
        result: 'u32',
        nonblocking: false,
      },
      webinix_is_app_running: {
        parameters: [],
        result: 'i32',
        nonblocking: false,
      },
...
export function new_server(win : Deno.Pointer = null, path : string) : string {
  load_lib();
  const ptr = webinix_lib.symbols.webinix_new_server(win, string_to_uint8array(path));
...
export function open(win, url : string, browser : number) : number {
  load_lib();
  return webinix_lib.symbols.webinix_open(win, string_to_uint8array(url), browser);
}
...
// the main thread. Lets do it in another way for now.
  ...
  export async function wait() {
    ...
    while(true) {
      await sleep(50);
      if(!webinix_lib.symbols.webinix_is_app_running())
        break;
    }
  }
// - - - - - - - - - - - - - - - -
...
export function run(win : number, javascript : js) {
  ...
  // Execute the script
  webinix_lib.symbols.webinix_script_interface(win, string_to_uint8array(javascript.Script), javascript.Timeout, bool_buffer, integer_buffer, pointer_buffer);
  ...
  // Clean memory
  webinix_lib.symbols.webinix_clean_mem(charPointer);
}
...
export function bind(win : number, element : string, func : Function) : number {
  ...
  const callbackResource = new Deno.UnsafeCallback(
	...
	webinix_lib.symbols.webinix_bind_interface(win, string_to_uint8array(element), callbackResource.pointer);
  return 0;
}
