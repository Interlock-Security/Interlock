...
/*
#cgo CFLAGS: -I ./
#cgo windows LDFLAGS: -L ./ -lwebinix-2-static-x64 -lws2_32
#cgo darwin LDFLAGS: -L ./ -lwebinix-2-static-x64 -lpthread -lm
#cgo linux LDFLAGS: -L ./ -lwebinix-2-static-x64 -lpthread -lm
#include <webinix.h>
extern void GoWebuiEvent(size_t _window, unsigned int _event_type, char* _element, char* _data, unsigned int _event_number);
static void GoWebuiEvents_handler(webinix_event_t* e) {
    GoWebuiEvent(e->window, e->event_type, e->element, e->data, e->event_number);
}
static void go_webinix_bind(size_t win, const char* element) {
    webinix_bind(win, element, GoWebuiEvents_handler);
}
*/
import "C"
...
func GoWebuiEvent(window C.size_t, _event_type C.uint, _element *C.char, _data *C.char, _event_number C.uint) {
	...
	var window_id uint = uint(C.webinix_interface_get_window_id(window))
	...
	if len(response) > 0 {
		...
		C.webinix_interface_set_response(window, _event_number, c_response)
	}
}
...
func Script(window uint, js *JavaScript, script string) bool {
	...
	status := C.webinix_script(C.size_t(window), c_script, C.uint(js.Timeout), ptr, C.size_t(uint64(js.BufferSize)))
...
func Run(window uint, script string) {
	...
	// Run the JavaScript
	C.webinix_run(C.size_t(window), c_script)
}
...
func SetRuntime(window uint, runtime uint) {
	...
	C.webinix_set_runtime(C.size_t(window), C.uint(runtime))
}
...
func NewWindow() uint {
	...
	// never change it. It's only managed by Webinix
	return uint(C.size_t(C.webinix_new_window()))
}
...
func IsShown(window uint) bool {
	...
	status := C.webinix_is_shown(C.size_t(window))
	return bool(status)
}
...
func Close(window uint) {
	...
	C.webinix_close(C.size_t(window))
}
...
func SetTimeout(seconds uint) {
	...
	C.webinix_set_timeout(C.uint(seconds))
}
...
func SetMultiAccess(window uint, access bool) {
	...
	C.webinix_set_multi_access(C.size_t(window), C._Bool(access))
}
...
func Show(window uint, content string) {
	...
	C.webinix_show(C.size_t(window), c_content)
}
...
func ShowBrowser(window uint, content string, browser uint) {
	...
	c_content := C.CString(content)
	C.webinix_show_browser(C.size_t(window), c_content, C.uint(browser))
}
...
func Bind(window uint, element string, callback func(Event) string) {
	...
	C.go_webinix_bind(C.size_t(window), c_element)

	// Get the window ID
	var window_id uint = uint(C.webinix_interface_get_window_id(C.size_t(window)))
