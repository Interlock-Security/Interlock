...
#include <webinix.h>
extern void go_webinix_event(void* _window, unsigned int _event_type, char* _element, char* _data, char* _response);
static void go_webinix_events_handler(webinix_event_t* e) {
    go_webinix_event(e->window, e->type, e->element, e->data, (char*)&e->response);
}
static void go_webinix_bind(void* win, const char* element) {
    webinix_bind(win, element, go_webinix_events_handler);
}
*/
import "C"
...
//
//export go_webinix_event
func go_webinix_event(window unsafe.Pointer, _event_type C.uint, _element *C.char, _data *C.char, _response *C.char) {
	...
	var window_id uint = uint(C.webinix_interface_get_window_id(unsafe.Pointer(window)))
	...
	if len(response) > 0 {
		...
		C.webinix_interface_set_response(_response, c_response)
	}
}
...
func Script(window unsafe.Pointer, js *JavaScript, script string) bool {
	...
	status := C.webinix_script(window, c_script, C.uint(js.Timeout), ptr, C.size_t(uint64(js.BufferSize)))
...
func NewWindow() unsafe.Pointer {
	...
	return unsafe.Pointer(C.webinix_new_window())
}
...
func Bind(window unsafe.Pointer, element string, callback func(Event) string) {
	...
	C.go_webinix_bind(window, c_element)
