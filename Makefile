...
WEBUI_BUILD_FLAGS := -m64 -o webinix.o -I "$(MAKEFILE_DIR)/include/" -c "$(MAKEFILE_DIR)/src/webinix.c"

# Output files
# The static output is the same for all platforms
# The dynamic output is platform dependent
LIB_STATIC_OUT := libwebinix-2-static-x64.a
...
ifeq ($(OS),Windows_NT)
	...
	ifeq ($(shell uname),Darwin)
		...
		LIB_DYN_OUT := webinix-2-x64.dylib
		ifeq ($(COMPILER),)
			COMPILER = clang
		endif
	else
...
#	Static Release
	@cd $(BUILD_DIR) \
	&& echo "Build Webinix library ($(COMPILER) release static)..." \
...
#	Dynamic Release
	@cd $(BUILD_DIR) \
	&& echo "Build Webinix library ($(COMPILER) release dynamic)..." \
	&& $(COMPILER) $(ARCH_TARGET) $(CIVETWEB_BUILD_FLAGS) $(CIVETWEB_DEFINE_FLAGS) -Os -fPIC \
	&& $(COMPILER) $(ARCH_TARGET) $(WEBUI_BUILD_FLAGS) -O3 -fPIC \
	&& $(COMPILER) $(ARCH_TARGET) -shared -o $(LIB_DYN_OUT) webinix.o civetweb.o $(LWS2_OPT)
#	Clean
ifeq ($(PLATFORM),windows)
	@strip --strip-unneeded $(BUILD_DIR)/$(LIB_DYN_OUT)
	@-cd $(BUILD_DIR) && del *.o >nul 2>&1
