...
namespace webinix{
	...
	class window{
		...
		bool show(const std::string * html, const webinix::custom_browser_t *b){

			// if(o_win.window_is_running())
			// 	return true;
			
			// Update custom browser reference
			webinix::set_custom_browser(b);

			this->p_html = html;
			return o_win.window_show(this->p_html, webinix::browser::custom);
		}

		unsigned short bind(const std::string& id, void (* func_ref)(webinix::event e)) const{
		...
		void bind_all(void (* func_ref)(webinix::event e)){
		...
		void close(){
			...
			o_win.run(" webinix_close_window(); ", 1);
