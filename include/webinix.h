...
typedef enum {
    ...
    // all windows. You can use `webinix_set_event_blocking()` for
    ...
    // Default: False
    multi_client,
    // Allow or prevent Webinix from adding `webinix_auth` cookies.
    // Webinix uses these cookies to identify clients and block 
...
/**
 * @brief Create a new Webinix window object.
 *
 * @return Returns the window number.
 *
 * @example size_t myWindow = webinix_new_window();
 */
WEBUI_EXPORT size_t webinix_new_window(void);

/**
 * @brief Create a new webinix window object using a specified window number.
 *
 * @param window_number The window number (should be > 0, and < WEBUI_MAX_IDS)
 *
 * @return Returns the same window number if success.
 *
 * @example size_t myWindow = webinix_new_window_id(123);
 */
WEBUI_EXPORT size_t webinix_new_window_id(size_t window_number);
 ...
 *
 * @example size_t myWindowNumber = webinix_get_new_window_id();
 */
 ...
 *
 * @example webinix_bind(myWindow, "myFunction", myFunction);
 */
 ...
 * 
 * @example size_t browserID = webinix_get_best_browser(myWindow);
 */
 ...
 *
 * @example webinix_show(myWindow, "<html>...</html>"); | 
 * webinix_show(myWindow, "index.html"); | webinix_show(myWindow, "http://...");
 */
 ...
 * @return Returns True if showing the window is successed.
 *
 * @example webinix_show_client(e, "<html>...</html>"); | 
 * webinix_show_client(e, "index.html"); | webinix_show_client(e, "http://...");
 */
WEBUI_EXPORT bool webinix_show_client(webinix_event_t* e, const char* content);

/**
 * @brief Same as `webinix_show()`. But using a specific web browser.
 *
 * @param window The window number
 ...
 * @return Returns True if showing the window is successed.
 *
 * @example webinix_show_browser(myWindow, "<html>...</html>", Chrome); |
 * webinix_show(myWindow, "index.html", Firefox);
 */
 ...
 *
 * @example webinix_show_wv(myWindow, "<html>...</html>"); | webinix_show_wv(myWindow,
 * "index.html"); | webinix_show_wv(myWindow, "http://...");
 */
WEBUI_EXPORT bool webinix_show_wv(size_t window, const char* content);
 ...
 *
 * @example webinix_set_kiosk(myWindow, true);
 */
 ...
 *
 * @example webinix_set_custom_parameters(myWindow, "--remote-debugging-port=9222");
 */
WEBUI_EXPORT void webinix_set_custom_parameters(size_t window, char *params);
 ...
 * @param status True or False
 *
 * @example webinix_set_high_contrast(myWindow, true);
 */
WEBUI_EXPORT void webinix_set_high_contrast(size_t window, bool status);
 ...
 * @param status True or False
 *
 * @example webinix_set_resizable(myWindow, true);
 */
WEBUI_EXPORT void webinix_set_resizable(size_t window, bool status);
 ...
 *
 * @example bool status = webinix_browser_exist(Chrome);
 */
WEBUI_EXPORT bool webinix_browser_exist(size_t browser);
 ...
 * @param window The window number
 *
 * @example webinix_close(myWindow);
 */
 ...
 * @param window The window number
 *
 * @example webinix_minimize(myWindow);
 */
WEBUI_EXPORT void webinix_minimize(size_t window);
 ...
 * @param window The window number
 *
 * @example webinix_maximize(myWindow);
 */
WEBUI_EXPORT void webinix_maximize(size_t window);
 ...
 * @param e The event struct
 *
 * @example webinix_close_client(e);
 */
WEBUI_EXPORT void webinix_close_client(webinix_event_t* e);
 ...
 * @param window The window number
 *
 * @example webinix_destroy(myWindow);
 */
WEBUI_EXPORT void webinix_destroy(size_t window);

/**
 * @brief Close all open windows. `webinix_wait()` will return (Break).
 *
 * @example webinix_exit();
 */
 ...
 *
 * @example webinix_set_root_folder(myWindow, "/home/Foo/Bar/");
 */
 ...
 * @param path The local folder full path
 *
 * @example webinix_set_default_root_folder("/home/Foo/Bar/");
 */
 ...
 * This deactivates any previous handler set with `webinix_set_file_handler_window`
 *
 * @param window The window number
 ...
 * @brief Set a custom handler to serve files. This custom handler should
 * return full HTTP header and body.
 * This deactivates any previous handler set with `webinix_set_file_handler`
 *
 * @param window The window number
 ...
 */
WEBUI_EXPORT void webinix_set_file_handler_window(size_t window, const void* (*handler)(size_t window, const char* filename, int* length));

/**
 * @brief Use this API to set a file handler response if your backend need async 
 * response for `webinix_set_file_handler()`.
 *
 * @param window The window number
 * @param response The response buffer
 * @param length The response size
 *
 * @example webinix_interface_set_response_file_handler(myWindow, buffer, 1024);
 */
WEBUI_EXPORT void webinix_interface_set_response_file_handler(size_t window, const void* response, int length);
 ...
 * @param window The window number
 *
 * @example webinix_is_shown(myWindow);
 */
 ...
 *
 * @example webinix_set_timeout(30);
 */
 ...
 *
 * @example webinix_set_icon(myWindow, "<svg>...</svg>", "image/svg+xml");
 */
 ...
 * 
 * @example char* base64 = webinix_encode("Foo Bar");
 */
 ...
 *
 * @example char* str = webinix_decode("SGVsbG8=");
 */
WEBUI_EXPORT char* webinix_decode(const char* str);

/**
 * @brief Safely free a buffer allocated by Webinix using `webinix_malloc()`.
 *
 * @param ptr The buffer to be freed
 *
 * @example webinix_free(myBuffer);
 */
WEBUI_EXPORT void webinix_free(void* ptr);

/**
 * @brief Safely allocate memory using the Webinix memory management system. It
 * can be safely freed using `webinix_free()` at any time.
 *
 * @param size The size of memory in bytes
 *
 * @example char* myBuffer = (char*)webinix_malloc(1024);
 */
 ...
 *
 * @example webinix_memcpy(myBuffer, myData, 64);
 */
WEBUI_EXPORT void webinix_memcpy(void* dest, void* src, size_t count);
 ...
 *
 * @example webinix_send_raw(myWindow, "myJavaScriptFunc", myBuffer, 64);
 */
 ...
 * @param size The raw data size in bytes
 *
 * @example webinix_send_raw_client(e, "myJavaScriptFunc", myBuffer, 64);
 */
WEBUI_EXPORT void webinix_send_raw_client(webinix_event_t* e, const char* function, 
    const void* raw, size_t size);

/**
 * @brief Set a window in hidden mode. Should be called before `webinix_show()`.
 *
 * @param window The window number
 * @param status The status: True or False
 *
 * @example webinix_set_hide(myWindow, True);
 */
 ...
 *
 * @example webinix_set_size(myWindow, 800, 600);
 */
 ...
 * @param height The window height
 *
 * @example webinix_set_minimum_size(myWindow, 800, 600);
 */
WEBUI_EXPORT void webinix_set_minimum_size(size_t window, unsigned int width, unsigned int height);
 ...
 *
 * @example webinix_set_position(myWindow, 100, 100);
 */
 ...
 * @param window The window number
 *
 * @example webinix_set_center(myWindow);
 */
WEBUI_EXPORT void webinix_set_center(size_t window);

/**
 * @brief Set the web browser profile to use. An empty `name` and `path` means
 * the default user profile. Need to be called before `webinix_show()`.
 *
 * @param window The window number
 * @param name The web browser profile name
 * @param path The web browser profile full path
 *
 * @example webinix_set_profile(myWindow, "Bar", "/Home/Foo/Bar"); |
 * webinix_set_profile(myWindow, "", "");
 */
WEBUI_EXPORT void webinix_set_profile(size_t window, const char* name, const char* path);

/**
 * @brief Set the web browser proxy server to use. Need to be called before `webinix_show()`.
 *
 * @param window The window number
 ...
 *
 * @example webinix_open_url("https://webinix.me");
 */
WEBUI_EXPORT void webinix_open_url(const char* url);
 ...
 *
 * @example webinix_navigate(myWindow, "http://domain.com");
 */
 ...
 * @param url Full HTTP URL
 *
 * @example webinix_navigate_client(e, "http://domain.com");
 */
WEBUI_EXPORT void webinix_navigate_client(webinix_event_t* e, const char* url);
 ...
 *
 * @example size_t id = webinix_get_parent_process_id(myWindow);
 */
WEBUI_EXPORT size_t webinix_get_parent_process_id(size_t window);
 ...
 *
 * @example size_t id = webinix_get_child_process_id(myWindow);
 */
 ...
 *
 * @example HWND hwnd = webinix_win32_get_hwnd(myWindow);
 */
WEBUI_EXPORT void* webinix_win32_get_hwnd(size_t window);

/**
 * @brief Get the network port of a running window.
 * This can be useful to determine the HTTP link of `webinix.js`
 *
 * @param window The window number
 * 
 * @return Returns the network port of the window
 *
 * @example size_t port = webinix_get_port(myWindow);
 */
WEBUI_EXPORT size_t webinix_get_port(size_t window);

/**
 * @brief Set a custom web-server/websocket network port to be used by Webinix.
 ...
 *
 * @example size_t port = webinix_get_free_port();
 */
 ...
 * `webinix_set_config(ui_event_blocking, ...)` to update all windows.
 *
 * @param window The window number
 * @param status The blocking status `true` or `false`
 *
 * @example webinix_set_event_blocking(myWindow, true);
 */
 ...
 *
 * @example webinix_set_frameless(myWindow, true);
 */
WEBUI_EXPORT void webinix_set_frameless(size_t window, bool status);
 ...
 *
 * @example webinix_set_transparent(myWindow, true);
 */
WEBUI_EXPORT void webinix_set_transparent(size_t window, bool status);
 ...
 *
 * @example const char* mime = webinix_get_mime_type("foo.png");
 */
WEBUI_EXPORT const char* webinix_get_mime_type(const char* file);

// -- SSL/TLS -------------------------

/**
 * @brief Set the SSL/TLS certificate and the private key content, both in PEM
 * format. This works only with `webinix-2-secure` library. If set empty Webinix
 ...
 *
 * @example bool ret = webinix_set_tls_certificate("-----BEGIN
 * CERTIFICATE-----\n...", "-----BEGIN PRIVATE KEY-----\n...");
 */
WEBUI_EXPORT bool webinix_set_tls_certificate(const char* certificate_pem, const char* private_key_pem);
 ...
 *
 * @example webinix_run(myWindow, "alert('Hello');");
 */
 ...
 * @param script The JavaScript to be run
 *
 * @example webinix_run_client(e, "alert('Hello');");
 */
WEBUI_EXPORT void webinix_run_client(webinix_event_t* e, const char* script);
 ...
 *
 * @example bool err = webinix_script(myWindow, "return 4 + 6;", 0, myBuffer, myBufferSize);
 */
WEBUI_EXPORT bool webinix_script(size_t window, const char* script, size_t timeout,
 ...
 * @return Returns True if there is no execution error
 *
 * @example bool err = webinix_script_client(e, "return 4 + 6;", 0, myBuffer, myBufferSize);
 */
WEBUI_EXPORT bool webinix_script_client(webinix_event_t* e, const char* script, size_t timeout,
    char* buffer, size_t buffer_length);
 ...
 *
 * @example size_t count = webinix_get_count(e);
 */
WEBUI_EXPORT size_t webinix_get_count(webinix_event_t* e);
 ...
 *
 * @example long long int myNum = webinix_get_int_at(e, 0);
 */
 ...
 * @return Returns argument as integer
 *
 * @example long long int myNum = webinix_get_int(e);
 */
 ...
 *
 * @example const char* myStr = webinix_get_string_at(e, 0);
 */
 ...
 * @return Returns argument as string
 *
 * @example const char* myStr = webinix_get_string(e);
 */
 ...
 *
 * @example bool myBool = webinix_get_bool_at(e, 0);
 */
 ...
 * @return Returns argument as boolean
 *
 * @example bool myBool = webinix_get_bool(e);
 */
 ...
 *
 * @example size_t argLen = webinix_get_size_at(e, 0);
 */
 ...
 * @return Returns size in bytes
 *
 * @example size_t argLen = webinix_get_size(e);
 */
 ...
 *
 * @example webinix_return_int(e, 123);
 */
 ...
 *
 * @example webinix_return_string(e, "Response...");
 */
 ...
 *
 * @example webinix_return_bool(e, true);
 */
 ...
 *
 * @example size_t id = webinix_interface_bind(myWindow, "myID", myCallback);
 */
WEBUI_EXPORT size_t webinix_interface_bind(size_t window, const char* element,
    void (*func)(size_t, size_t, char*, size_t, size_t));

/**
 * @brief When using `webinix_interface_bind()`, you may need this function to easily set a response.
 *
 * @param window The window number
 * @param event_number The event number
 * @param response The response as string to be send to JavaScript
 *
 * @example webinix_interface_set_response(myWindow, e->event_number, "Response...");
 */
 ...
 *
 * @example bool status = webinix_interface_is_app_running();
 */
 ...
 *
 * @example size_t id = webinix_interface_get_window_id(myWindow);
 */
 ...
 * @return Returns argument as string
 *
 * @example const char* myStr = webinix_interface_get_string_at(myWindow, e->event_number, 0);
 */
WEBUI_EXPORT const char* webinix_interface_get_string_at(size_t window, size_t event_number, size_t index);
 ...
 * @return Returns argument as integer
 *
 * @example long long int myNum = webinix_interface_get_int_at(myWindow, e->event_number, 0);
 */
WEBUI_EXPORT long long int webinix_interface_get_int_at(size_t window, size_t event_number, size_t index);
 ...
 * @return Returns argument as float
 *
 * @example double myFloat = webinix_interface_get_int_at(myWindow, e->event_number, 0);
 */
WEBUI_EXPORT double webinix_interface_get_float_at(size_t window, size_t event_number, size_t index);
 ...
 * @return Returns argument as boolean
 *
 * @example bool myBool = webinix_interface_get_bool_at(myWindow, e->event_number, 0);
 */
WEBUI_EXPORT bool webinix_interface_get_bool_at(size_t window, size_t event_number, size_t index);
 ...
 * @return Returns size in bytes
 *
 * @example size_t argLen = webinix_interface_get_size_at(myWindow, e->event_number, 0);
 */
WEBUI_EXPORT size_t webinix_interface_get_size_at(size_t window, size_t event_number, size_t index);
 ...
 * webinix_show_client(e, "index.html"); | webinix_show_client(e, "http://...");
 */
WEBUI_EXPORT bool webinix_interface_show_client(size_t window, size_t event_number, const char* content);
 ...
 * @example webinix_close_client(e);
 */
WEBUI_EXPORT void webinix_interface_close_client(size_t window, size_t event_number);
 ...
 * @example webinix_send_raw_client(e, "myJavaScriptFunc", myBuffer, 64);
 */
WEBUI_EXPORT void webinix_interface_send_raw_client(size_t window, size_t event_number, const char* function, const void* raw, size_t size);
 ...
 * @example webinix_navigate_client(e, "http://domain.com");
 */
WEBUI_EXPORT void webinix_interface_navigate_client(size_t window, size_t event_number, const char* url);
 ...
 * @example webinix_run_client(e, "alert('Hello');");
 */
WEBUI_EXPORT void webinix_interface_run_client(size_t window, size_t event_number, const char* script);
 ...
 * @example bool err = webinix_script_client(e, "return 4 + 6;", 0, myBuffer, myBufferSize);
 */
WEBUI_EXPORT bool webinix_interface_script_client(size_t window, size_t event_number, const char* script, size_t timeout, char* buffer, size_t buffer_length);
