...
/**
 * @brief Get a free window number that can be used with `webinix_new_window_id()`.
 *
 ...
 *
 * @example webinix_show(myWindow, "<html>...</html>"); | webinix_show(myWindow, "index.html");
 */
 ...
 * @return Returns True if showing the window is successed.
 *
 * @example webinix_show_browser(myWindow, "<html>...</html>", Chrome); | webinix_show(myWindow, "index.html", Firefox);
 */
...
/**
 * @brief Set the web-server root folder path for all windows. Should be used before `webinix_show()`.
 *
 * @param path The local folder full path
 *
 ...
 *
 * @example webinix_encode("SGVsbG8=");
 */
...
/**
 * @brief Safely allocate memory using the Webinix memory management system. It can be safely freed using `webinix_free()`
 * at any time.
 *
 * @param size The size of memory in bytes
 *
 * @example webinix_malloc(1024);
 */
 ...
 * called before `webinix_show()`.
 *
 * @param window The window number
 * @param name The web browser profile name
 * @param path The web browser profile full path
 *
 * @example webinix_set_profile(myWindow, "Bar", "/Home/Foo/Bar"); | webinix_set_profile(myWindow, "", "");
 */
...
/**
 * @brief Set the SSL/TLS certificate and the private key content, both in PEM format. This works only with `webinix-2-secure` library. If set empty Webinix will generate a self-signed certificate.
 *
 ...
 *
 * @example bool ret = webinix_set_tls_certificate("-----BEGIN CERTIFICATE-----\n...", "-----BEGIN PRIVATE KEY-----\n...");
 */
...
// `webinix_bind()`. The func is (Window, EventType, Element, EventNumber, BindID).
WEBUI_EXPORT size_t
    webinix_interface_bind(size_t window, const char* element, void (*func)(size_t, size_t, char*, size_t, size_t));
