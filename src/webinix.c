...
static const char* webinix_html_res_not_available =
    ...
    "href=\"https://www.webinix.me\"><small>Webinix "
...
static const char* webinix_nodejs_not_found =
    ...
    "a></p><br><a "
    "href=\"https://www.webinix.me\"><small>Webinix v" WEBUI_VERSION "<small></a></body></html>";
static const char* webinix_def_icon = "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"64\" height=\"64\" "
...
// Call cb
	...
	printf(
	    "[Core]\t\t_webinix_interface_bind_handler() -> user-callback response "
	    "[%s]\n",
	    response
	);
#endif
...
// Generate the full Webinix Bridge
	...
	int c = sprintf(
	    ...
	    "globalThis.webinix = new WebuiBridge({ "
...
#ifdef WEBUI_LOG
	printf(
	    "[Core]\t\t_webinix_browser_create_new_profile(%zu) -> Generating Webinix "
	    "profile...\n",
	    browser
	);
#endif
...
static bool _webinix_show_window(_webinix_window_t* win, const char* content, int type, size_t browser) {
	...
	if (type == WEBUI_SHOW_HTML) {
		...
		char* url_encoded = (char*)_webinix_malloc(64 + _webinix_strlen(file_url_encoded)); // [http][domain][port] [file_encoded]
...
// New server thread
		...
		// Send the packet
		_webinix_send(win, win->token, 0, WEBUI_CMD_NAVIGATION, (const char*)window_url, _webinix_strlen(window_url));
...
// Call user cb
#ifdef WEBUI_LOG
				printf("[Core]\t\t_webinix_window_event() -> Calling user "
				       "callback...\n[Call]\n");
#endif
				e.bind_id = cb_index;
...
#ifdef WEBUI_LOG
				printf(
				    "[Core]\t\t_webinix_http_handler() -> Trying to interpret local "
				    "file (Runtime = "
				    "%zu)\n",
				    win->runtime
				);
...
#ifdef WEBUI_LOG
				printf(
				    "[Core]\t\t_webinix_receive()... > Multi packet started, "
				    "Expecting %zu bytes...\n",
				    expect_len
				);
#endif
...
#ifdef WEBUI_LOG
				printf(
				    "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Invalid "
				    "Packet.\n",
				    recvNum
				);
#endif
