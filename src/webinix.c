...
static bool _webinix_browser_create_new_profile(_webinix_window_t* win, size_t browser) {
    ...
    if (browser == Chrome) {
        ...
        if (!win->custom_profile){
            if(!win->disable_browser_high_contrast)
                WEBUI_SN_PRINTF_DYN(win->profile_path, WEBUI_MAX_PATH, "%s%s.Webinix%sWebinixFirefoxProfile", temp, os_sep, os_sep);
            else
                WEBUI_SN_PRINTF_DYN(win->profile_path, WEBUI_MAX_PATH, "%s%s.Webinix%sWebinixFirefoxProfile-NoHC", temp, os_sep, os_sep);
        }
