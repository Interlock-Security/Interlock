...
bool _webinix_deno_exist() {
    ...
    if(_webinix_cmd_sync("deno --version") == 0) {
    ...
    if(browser == webinix.browser.firefox) {
        ...
        if(!_webinix_folder_exist(buf)) {
            ...
            if(!_webinix_folder_exist(buf)) {
                ...
                _webinix_cmd_sync(buf); // Create directory
            }
            sprintf(buf, "%s%schrome%suserChrome.css", firefox_profile_path, webinix_sep, webinix_sep, webinix_sep);
            file = fopen(buf, "a");
            if(file == NULL)
                return false;
            #ifdef _WIN32
...
bool _webinix_browser_exist(webinix_window_t* win, unsigned int browser) {
    ...
    if(browser == webinix.browser.firefox) {
        ...
        // Firefox
            ...
            if(_webinix_cmd_sync("firefox -v") == 0) {
        ...
        // Chrome
            ...
            // Chrome on Linux
            if(_webinix_cmd_sync("google-chrome --version") == 0) {
...
int _webinix_cmd_async(char* cmd) {
    ...
    int res = 0;
        ...
        // Run in silent
        res = _webinix_cmd_sync(buf);
    #else
        // Make command async
        if(fork() >= 0) {
            // Run in silent
            _webinix_cmd_sync(cmd);
            return 0;
        }
        else
            res = 1;
...
int _webinix_run_browser(webinix_window_t* win, char* cmd) {
    ...
    if(win->core.detect_process_close) {
        ...
        // TODO: We need to set 'res = 1' if _webinix_run_browser_detect_proc_task() fails. 
    }
    else {

        // Run a async command

        res = _webinix_cmd_async(cmd);
    }
...
bool _webinix_browser_start_firefox(webinix_window_t* win, const char* address) {
    ...
    char full[1024];
    sprintf(full, "%s -P Webinix -private -new-window %s", win->core.browser_path, address);
