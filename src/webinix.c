...
static void _webinix_window_event(_webinix_window_t* win, int event_type, char* element, size_t event_number, char* webinix_internal_id);
...
void webinix_delete_profile(size_t window) {
    ...
    if (_webinix_folder_exist(win->profile_path)) {
        ...
        if (win->current_browser == Firefox) {
            ...
            #ifdef _WIN32
                // Windows
                _webinix_remove_firefox_ini_profile("%APPDATA%\\Mozilla\\Firefox\\profiles.ini", win->profile_name);
            ...
            #else
                // macOS
                _webinix_remove_firefox_ini_profile("~/Library/Application Support/Firefox/profiles.ini", win->profile_name);
                _webinix_delete_folder(win->profile_path);
            #endif
        }
        else {
...
const char* webinix_get_string_at(webinix_event_t* e, size_t index) {
    ...
    // Dereference
    if (_webinix_core.exit_now || _webinix_core.wins[e->window] == NULL) return NULL;
...
size_t webinix_get_size_at(webinix_event_t* e, size_t index) {
    ...
    // Dereference
    if (_webinix_core.exit_now || _webinix_core.wins[e->window] == NULL) return 0;
    _webinix_window_t* win = _webinix_core.wins[e->window];
...
static bool _webinix_browser_create_new_profile(_webinix_window_t* win, size_t browser) {
    ...
    if (browser == Chrome) {
        ...
        if (!_webinix_folder_exist(win->profile_path) || !_webinix_is_firefox_ini_profile_exist(path_ini, win->profile_name)) {
...
static int _webinix_http_handler(struct mg_connection *conn, void *_win) {
    ...
    if (strcmp(ri->request_method, "GET") == 0) {
        ...
        if (strcmp(url, "/webinix.js") == 0) {
            ...
            //     #ifdef WEBUI_LOG
            //         printf("[Core]\t\t_webinix_http_handler() -> Local file .HTML Already Handled (403)\n");
            //     #endif
            ...
            // else {
                ...
                if (win->runtime != None) {
                    ...
                    #ifdef WEBUI_LOG
                        printf("[Core]\t\t_webinix_http_handler() -> Trying to interpret local file (Runtime = %zu)\n", win->runtime);
                    #endif
                ...
                if (!_webinix_core.exit_now) { // Check if previous event called exit()
                    ...
                    if ((unsigned char)packet[WEBUI_PROTOCOL_CMD] == WEBUI_CMD_CLICK) {
                        ...
                        while (token != NULL) {
                            ...
                            #ifdef WEBUI_LOG
                                printf("[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Argument %zu: %zu bytes\n", recvNum, token_num, arg_len);
                            #endif
                        ...
                        if(data_size_expected == data_size_recv) {
                            ...
                            #ifdef WEBUI_LOG
                                printf("[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Expected and received %zu bytes of data.\n", recvNum, data_size_expected);
                            #endif
                            ...
                            // Send the packet
                            _webinix_send(win, win->token, packet_id, WEBUI_CMD_CALL_FUNC, event_inf->response, _webinix_strlen(event_inf->response));
                            ...
                            #ifdef WEBUI_LOG
                                printf("[Core]\t\t[Thread %zu] _webinix_receive_thread() -> No enough data received. Expected %zu bytes, received %zu bytes.\n", recvNum, data_size_expected, data_size_recv);
                            #endif
                            ...
                            // Send the packet
                            _webinix_send(win, win->token, packet_id, WEBUI_CMD_CALL_FUNC, NULL, 0);
                        }
