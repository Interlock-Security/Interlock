...
// -- Heap ----------------------------
static const char* webinix_html_served = "<html><head><title>Access Denied</title><style>html{-ms-text-size-adjust:100%;-webkit-text-size-adjust:100%}body{background-color:#2F2F2F;font-family:sans-serif;margin:20px;color:#00ff00}a{color:#00ff00}</style></head><body><h2>[ ! ] Access Denied</h2><p>You can't access this window<br>because it's already served.<br><br>The security policy is set to<br>deny multiple requests.</p><br><a href=\"https://www.webinix.me\"><small>Webinix Library<small></a></body></html>";
static const char* webinix_html_res_not_available = "<html><head><title>Resource Not Available</title><style>html{-ms-text-size-adjust:100%;-webkit-text-size-adjust:100%}body{background-color:#2F2F2F;font-family:sans-serif;margin:20px;color:#00ff00}a{color:#00ff00}</style></head><body><h2>[ ! ] Resource Not Available</h2><p>The requested resource is not available.</p><br><a href=\"https://www.webinix.me\"><small>Webinix Library<small></a></body></html>";
...
bool _webinix_browser_create_profile_folder(webinix_window_t* win, unsigned int browser) {
    ...
    if(!_webinix_browser_folder_exist(parm)) {
        ...
        for(unsigned int n = 0; n <= (webinix.startup_timeout * 2); n++) {
...
bool _webinix_browser_start_chrome(webinix_window_t* win, char* address) {
...
bool _webinix_browser_start_custom(webinix_window_t* win, char* address) {
...
bool _webinix_browser_start_firefox(webinix_window_t* win, char* address) {
...
bool _webinix_browser_start_edge(webinix_window_t* win, char* address) {
...
bool _webinix_browser_start(webinix_window_t* win, char* address, unsigned int browser) {
...
const char* webinix_new_server(webinix_window_t* win, const char* html) {
...
void _webinix_window_send(webinix_window_t* win, char* packet, size_t packets_size) {
    ...
    #ifdef WEBUI_LOG
        printf("[%d] _webinix_window_send([%.*s], [%d])... [ ", win->core.window_number, packets_size, packet, packets_size);
...
bool webinix_open(webinix_window_t* win, char* url, unsigned int browser) {
