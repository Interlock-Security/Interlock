...
static bool _webinix_show_window(_webinix_window_t * win, const char * content, int type, size_t browser) {
    ...
    if (type == WEBUI_SHOW_HTML) {
        ...
        win->is_embedded_html = true;
        char * refresh = (char * ) _webinix_malloc(64 + strlen(user_url));
        ...
        char * url_encoded =
            (char * ) _webinix_malloc(64 + _webinix_strlen(file_url_encoded)); // [http][domain][port] [file_encoded]
...
/*
    static char* _webinix_inject_bridge(_webinix_window_t* win, const char
   *user_html) {
        ...
        if (strcmp(url, "/webinix.js") == 0) {
            ...
            if (win->is_embedded_html) {
                ...
                if (win->html_handled) {
                    ...
                    #ifdef WEBUI_LOG
                    printf("[Core]\t\t_webinix_http_handler() -> Index local file Already "
                        "Handled "
                        "(403)\n");
                    #endif
            ...
            //     #ifdef WEBUI_LOG
            //         printf("[Core]\t\t_webinix_http_handler() -> Local file .HTML
            //         Already Handled (403)\n");
            //     #endif

            //     _webinix_http_send_error_page(
            //         conn,         //         webinix_html_served,
            //         403
            //     );
...
static void _webinix_receive(_webinix_window_t * win, int event_type, void * data, size_t len) {
    ...
    if (multi_packet) {
        if ((multi_receive + len) > multi_expect) {
            ...
            #ifdef WEBUI_LOG
            printf(
                "[Core]\t\t_webinix_receive()... > Multi packet received more data "
                "than expected (%zu + %zu "
                "> %zu).\n",
            ...
            if (packet_len >= WEBUI_PROTOCOL_SIZE &&
                ...
                if (!_webinix_core.exit_now) { // Check if previous event called exit()
                    ...
                    if ((unsigned char) packet[WEBUI_PROTOCOL_CMD] == WEBUI_CMD_CLICK) {
                        ...
                        printf(
                            "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Call ID: "
                            "[%u] "
                            "\n",
                            recvNum, packet_id
                        );
