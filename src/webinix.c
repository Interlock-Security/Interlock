...
size_t webinix_new_window_id(size_t window_number) {
    ...
    if (_webinix_is_empty(_webinix_core.default_server_root_path))
        ...
        WEBUI_SPF(win->server_root_path, WEBUI_MAX_PATH, "%s", _webinix_core.default_server_root_path);
    ...
    printf("[User] webinix_new_window_id() -> New window Token 0x%08X (%"
        PRIu32 ")\n", win->token, win->token);
    #endif
...
size_t webinix_bind(size_t window, const char* element, void( * func)(webinix_event_t* e)) {
    ...
    WEBUI_SPF(webinix_internal_id, (bf_len), "%zu/%s", win->window_number, element);
...
void webinix_config(webinix_config option, bool status) {
...
bool webinix_set_default_root_folder(const char* path) {
    ...
    #endif
    WEBUI_SPF(_webinix_core.default_server_root_path, WEBUI_MAX_PATH, "%s", path);

    // Update all windows. This will works only
    // for non-running windows.
    for (size_t i = 1; i <= _webinix_core.last_win_number; i++) {
        if (_webinix_core.wins[i] != NULL) {
            WEBUI_SPF(_webinix_core.wins[i]->server_root_path, WEBUI_MAX_PATH, 
...
static void _webinix_free_mem(void * ptr) {
    ...
    for (size_t i = _webinix_core.ptr_position; i >= 0; i--) {
...
static bool _webinix_file_exist_mg(_webinix_window_t * win, struct mg_connection * conn) {
    ...
    WEBUI_SPF(full_path, bf_len, "%s%s%s", win->server_root_path, webinix_sep, file);
...
static char* _webinix_get_full_path_from_url(_webinix_window_t * win, const char* file, const char* url) {
    ...
    #ifdef WEBUI_LOG
    printf("[Core]\t\t_webinix_get_full_path_from_url() -> Full path: [%s]\n", full_path);
    #endif
...
static int _webinix_serve_file(_webinix_window_t * win, struct mg_connection * conn) {
    ...
    if (http_status_code != 200) {
        ...
        char* full_path = _webinix_get_full_path_from_url(win, file, url);
...
static int _webinix_interpret_file(_webinix_window_t * win, struct mg_connection * conn, char* index) {
    ...
    if (index != NULL && !_webinix_is_empty(index)) {
        ...
        // Get full path
        full_path = _webinix_get_full_path_from_url(win, file, url);
    }
...
static const char* _webinix_generate_js_bridge(_webinix_window_t * win) {
    ...
    for (size_t i = 1; i < WEBUI_MAX_IDS; i++) {
        if (_webinix_core.html_elements[i] != NULL && !_webinix_is_empty(_webinix_core.html_elements[i])) {
            WEBUI_SCAT(event_cb_js_array, cb_mem_size, "\"");
            WEBUI_SCAT(event_cb_js_array, cb_mem_size, _webinix_core.html_elements[i]);
            WEBUI_SCAT(event_cb_js_array, cb_mem_size, "\",");
        }
    }
    WEBUI_SCAT(event_cb_js_array, cb_mem_size, "]");
    ...
    int c = WEBUI_SPF(
        ...
        webinix_javascript_bridge, TLS, token, win->ws_port, win->window_number, event_cb_js_array, log
    );
    // Window Size
    if (win->size_set)
...
static bool _webinix_browser_create_new_profile(_webinix_window_t * win, size_t browser) {
    ...
    if (browser == Chrome) {
        ...
        // Google Chrome
        if (!win->custom_profile)
            WEBUI_SPF(win->profile_path, WEBUI_MAX_PATH, "%s%s.Webinix%sWebinixChromeProfile", temp, webinix_sep, webinix_sep);
        return true;
    } else if (browser == Edge) {

        // Edge
        if (!win->custom_profile)
            WEBUI_SPF(win->profile_path, WEBUI_MAX_PATH, "%s%s.Webinix%sWebinixEdgeProfile", temp, webinix_sep, webinix_sep);
        return true;
    } else if (browser == Epic) {

        // Epic
        if (!win->custom_profile)
            WEBUI_SPF(win->profile_path, WEBUI_MAX_PATH, "%s%s.Webinix%sWebinixEpicProfile", temp, webinix_sep, webinix_sep);
        return true;
    } else if (browser == Vivaldi) {

        // Vivaldi
        if (!win->custom_profile)
            WEBUI_SPF(win->profile_path, WEBUI_MAX_PATH, "%s%s.Webinix%sWebinixVivaldiProfile", temp, webinix_sep, webinix_sep);
        return true;
    } else if (browser == Brave) {

        // Brave
        if (!win->custom_profile)
            WEBUI_SPF(win->profile_path, WEBUI_MAX_PATH, "%s%s.Webinix%sWebinixBraveProfile", temp, webinix_sep, webinix_sep);
        return true;
    } else if (browser == Yandex) {

        // Yandex
        if (!win->custom_profile)
            WEBUI_SPF(win->profile_path, WEBUI_MAX_PATH, "%s%s.Webinix%sWebinixYandexProfile", temp, webinix_sep, webinix_sep);
        return true;
    } else if (browser == Chromium) {

        // Chromium
        if (!win->custom_profile)
            WEBUI_SPF(win->profile_path, WEBUI_MAX_PATH, "%s%s.Webinix%sWebinixChromiumProfile", temp, webinix_sep, webinix_sep);
        return true;
        ...
        #endif
        if (!win->custom_profile)
            WEBUI_SPF(win->profile_path, WEBUI_MAX_PATH, "%s%s.Webinix%sWebinixFirefoxProfile", temp, webinix_sep, webinix_sep);
        ...
        if (!_webinix_folder_exist(win->profile_path) ||
            ...
            // prefs.js
            FILE * file;
            WEBUI_SPF(buf, sizeof(buf), "%s%sprefs.js", win->profile_path, webinix_sep);
            ...
            // userChrome.css
            WEBUI_SPF(buf, sizeof(buf), "\"%s%schrome%s\"", win->profile_path, webinix_sep, webinix_sep);
            if (!_webinix_folder_exist(buf)) {

                WEBUI_SPF(buf, sizeof(buf), "mkdir \"%s%schrome%s\"", win->profile_path, webinix_sep, webinix_sep);
                _webinix_cmd_sync(win, buf, false); // Create directory
            }
            WEBUI_SPF(buf, sizeof(buf), "%s%schrome%suserChrome.css", win->profile_path, webinix_sep, webinix_sep);
            WEBUI_FOPEN(file, buf, "a");
            if (file == NULL)
                return false;
            #ifdef _WIN32
...
static char* _webinix_generate_internal_id(_webinix_window_t * win, const char* element) {
    ...
    WEBUI_SPF(webinix_internal_id, internal_id_size, "%zu/%s", win->window_number, element);
...
static int _webinix_tls_initialization(void * ssl_ctx, void * ptr) {
    ...
    if (SSL_CTX_use_certificate(ctx, cert) <= 0) {
        #ifdef WEBUI_LOG
        printf("[Core]\t\t_webinix_tls_initialization() -> SSL_CTX_use_certificate "
            "failed\n");
        #endif
        _webinix_panic();
        return -1;
    ...
    if (private_key == NULL) {
        #ifdef WEBUI_LOG
        printf("[Core]\t\t_webinix_tls_initialization() -> PEM_read_bio_PrivateKey "
            "failed\n");
        #endif
        _webinix_panic();
        return -1;
    ...
    if (SSL_CTX_use_PrivateKey(ctx, private_key) <= 0) {
        #ifdef WEBUI_LOG
        printf("[Core]\t\t_webinix_tls_initialization() -> SSL_CTX_use_PrivateKey "
            "failed\n");
        #endif
        _webinix_panic();
        return -1;
        ...
        if (strcmp(url, "/webinix.js") == 0) {
            ...
            if (win->is_embedded_html) {
                ...
                if (win->html_handled) {
                    ...
                    #ifdef WEBUI_LOG
                    printf("[Core]\t\t_webinix_http_handler() -> Index local file Already Handled (403)\n");
                    #endif
                    ...
                    // Set full path
                    // [Path][Sep][File Name]
                    size_t bf_len = (_webinix_strlen(win->server_root_path) + 1 + 8);
                    char* index = (char*)_webinix_malloc(bf_len);

                    // Index.ts
                    WEBUI_SPF(index, bf_len, "%s%sindex.ts", win->server_root_path, webinix_sep);
                    ...
                    // Index.js
                    WEBUI_SPF(index, bf_len, "%s%sindex.js", win->server_root_path, webinix_sep);
                    if (_webinix_file_exist(index)) {
...
static WEBUI_THREAD_SERVER_START {
                ...
                if (!_webinix_mutex_is_connected(win, WEBUI_MUTEX_NONE)) {
                    ...
                    if (!_webinix_mutex_is_connected(win, WEBUI_MUTEX_NONE) && win->server_handled) {
                        ...
                        do {
                            #ifdef WEBUI_LOG
                            printf(
                                "[Core]\t\t_webinix_server_thread([%zu]) -> Waiting for "
                                "first "
                                "connection\n",
                                win->window_number
                            );
    ...
    _webinix_free_mem((void*)ws_port);
    _webinix_mutex_is_connected(win, WEBUI_MUTEX_FALSE);
...
static void _webinix_receive(_webinix_window_t * win, int event_type, void * data, size_t len) {
    ...
    if (multi_packet) {
        if ((multi_receive + len) > multi_expect) {
            ...
            #ifdef WEBUI_LOG
            printf(
                "[Core]\t\t_webinix_receive() -> Multi packet received "
...
static WEBUI_THREAD_RECEIVE {
    ...
    if (!_webinix_mutex_is_exit_now(WEBUI_MUTEX_NONE)) {
        ...
        if (event_type == WEBUI_WS_DATA) {
            ...
            #ifdef WEBUI_LOG
            printf(
                "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Data "
                "received\n",
                recvNum
            );
            printf(
                "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Packet Size : "
                "%zu bytes\n",
                recvNum, packet_len
            );
            printf(
                "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Packet Header "
                ": [ ",
                recvNum
            );
            ...
            printf("]\n");
            printf(
                "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Packet Token: "
                "0x%08X (%"
                PRIu32 ")\n",
                recvNum, packet_token, packet_token
            );
            printf(
                "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Packet ID: "
                "0x%04X (%u)\n",
                recvNum, packet_id, packet_id
            );
            printf("[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Packet Data: [", recvNum);
            //_webinix_print_ascii(&packet[WEBUI_PROTOCOL_DATA], (packet_len -
            // WEBUI_PROTOCOL_SIZE));
            printf("]\n");
            #endif
            ...
            if (packet_len >= WEBUI_PROTOCOL_SIZE &&
                ...
                if (!_webinix_mutex_is_exit_now(WEBUI_MUTEX_NONE)) { // Check if previous event called exit()
                    ...
                    if ((unsigned char) packet[WEBUI_PROTOCOL_CMD] == WEBUI_CMD_CLICK) {
                        ...
                        printf(
                            "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Element "
                            "size: "
                            "%zu bytes \n",
                            recvNum, element_len
                        );
                        ...
                        if (win->has_events) {
                            ...
                            #ifdef WEBUI_LOG
                            printf(
                                "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> WEBUI_CMD_NAVIGATION \n",
                                recvNum
                            );
                            printf(
                                "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> URL size: %zu bytes \n",
                                recvNum, url_len
                            );
                            printf(
                                "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> URL: [%s] \n",
                                recvNum, url
                            );
                            #endif
                        ...
                        #ifdef WEBUI_LOG
                        printf(
                            "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> WEBUI_CMD_CALL_FUNC \n",
                            recvNum
                        );
                        printf(
                        ...
                        while(token != NULL && token_num < WEBUI_MAX_ARG + 1) {
                            ...
                            #ifdef WEBUI_LOG
                            printf(
                                "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Argument %zu: %zu bytes\n",
                        ...
                        if (data_size_expected == data_size_recv) {
                            ...
                            #ifdef WEBUI_LOG
                            printf(
                                "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Expected and received %zu bytes of data.\n",
                            ...
                            if (cb_index > 0 && _webinix_core.cb[cb_index] != NULL) {
                                ...
                                #ifdef WEBUI_LOG
                                printf(
                                    "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Calling user callback\n[Call]\n",
                                    recvNum
                                );
                                #endif
                                e.bind_id = cb_index;
                            ...
                            #ifdef WEBUI_LOG
                            printf(
                                "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> user-callback response [%s]\n",
                        ...
                        printf(
                            "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Unknown "
                            "command "
                            "[0x%02x]\n",
                    ...
                    printf(
                        "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Window "
                        "disconnected.\n",
                        recvNum
                    );
                }
                ...
                #ifdef WEBUI_LOG
                printf(
                    "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Invalid Packet.\n",
                    recvNum
                );
                #endif
            ...
            #ifdef WEBUI_LOG
            printf(
                "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> WebSocket "
                "connected\n",
                recvNum
            );
            #endif
            ...
            printf(
                "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> UNKNOWN EVENT "
                "TYPE (%zu)\n",
                recvNum, event_type
            );
        }
...
// -- WebView -------------------------
    ...
    static WEBUI_THREAD_WEBVIEW {
        ...
        if (!_webinix_core.webview_cacheFolder) {
            ...
            WEBUI_SPF(_webinix_core.webview_cacheFolder, WEBUI_MAX_PATH, 
