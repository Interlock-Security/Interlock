...
#ifdef WEBUI_TLS
bool _webinix_generate_self_signed_cert(char* ssl_cert, char* ssl_key);
#endif
...
#ifdef WEBUI_LOG
        printf("[Core]\t\t_webinix_generate_self_signed_cert()...\n");
#endif
        int ret = 0;
        int bits = 2048;
        int serial = 0;
...
static bool _webinix_show_window(_webinix_window_t* win, const char* content, bool is_embedded_html, size_t browser) {
	...
	if(_webinix_is_empty(_webinix_core.ssl_cert) || _webinix_is_empty(_webinix_core.ssl_key)) {
		...
		if(!_webinix_generate_self_signed_cert(ssl_cert, ssl_key)) {
#ifdef WEBUI_LOG
