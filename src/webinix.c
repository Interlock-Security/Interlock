...
static bool _webinix_show_window(_webinix_window_t* win, const char* content, bool is_embedded_html, size_t browser);
...
static int _webinix_get_browser_args(_webinix_window_t* win, size_t browser, char* buffer, size_t max);
...
static const char* webinix_html_served =
    ...
    "href=\"https://www.webinix.me\"><small>Webinix v" WEBUI_VERSION "<small></a></body></html>";
static const char* webinix_html_res_not_available =
    ...
    "Available</h2><p>The requested resource is not available.</p><br><a href=\"https://www.webinix.me\"><small>Webinix "
...
static const char* webinix_deno_not_found =
    ...
    "href=\"https://www.webinix.me\"><small>Webinix v" WEBUI_VERSION "<small></a></body></html>";
static const char* webinix_nodejs_not_found =
    ...
    "href=\"https://www.webinix.me\"><small>Webinix v" WEBUI_VERSION "<small></a></body></html>";
static const char* webinix_def_icon =
...
void webinix_run(size_t window, const char* script) {
	...
	// Dereference
	if (_webinix_core.exit_now || _webinix_core.wins[window] == NULL)
		return;
...
void webinix_delete_profile(size_t window) {
	...
	if (_webinix_folder_exist(win->profile_path)) {
		...
		if (win->current_browser == Firefox) {
			...
			_webinix_remove_firefox_profile_ini("~/snap/firefox/common/.mozilla/firefox/profiles.ini", win->profile_name);
			_webinix_delete_folder(win->profile_path);
#else
			// macOS
			_webinix_remove_firefox_profile_ini(
...
const char* webinix_get_string_at(webinix_event_t* e, size_t index) {
	...
	// Dereference
	if (_webinix_core.exit_now || _webinix_core.wins[e->window] == NULL)
		return NULL;
...
static bool _webinix_check_certificate(const char* certificate_pem, const char* private_key_pem) {
...
#ifdef WEBUI_LOG
		printf("[Core]\t\t_webinix_check_certificate() -> SSL_CTX_use_certificate failed\n");
#endif
		BIO_free_all(bio_cert);
		X509_free(cert);
		SSL_CTX_free(ctx);
	...
	if (private_key == NULL) {
#ifdef WEBUI_LOG
		printf("[Core]\t\t_webinix_check_certificate() -> PEM_read_bio_PrivateKey failed\n");
#endif
...
#ifdef WEBUI_LOG
		printf("[Core]\t\t_webinix_check_certificate() -> SSL_CTX_use_PrivateKey failed\n");
#endif
		EVP_PKEY_free(private_key);
		BIO_free_all(bio_key);
		BIO_free_all(bio_cert);
	...
	for (size_t i = 1; i <= _webinix_core.last_win_number; i++) {
		if (_webinix_core.wins[i] != NULL) {
			if (_webinix_core.wins[i]->connected) {
				...
				// Send the packet
				_webinix_send(
				    _webinix_core.wins[i], _webinix_core.wins[i]->token, 0, WEBUI_CMD_CLOSE, NULL, 0
				);
			}
		}
	}
...
#ifdef WEBUI_LOG
	printf("[Core]\t\t_webinix_get_file_name_from_url() -> Full path: [%s]\n", full_path);
#endif
	...
	if (strcmp(extension, "ts") == 0 || strcmp(extension, "js") == 0) {
		...
		if (win->runtime == Deno) {
			...
			if (_webinix_deno_exist(win)) {
				...
				char* cmd = (char*)_webinix_malloc(64 + _webinix_strlen(full_path));
#ifdef _WIN32
				sprintf(
				    cmd,
			...
			if (_webinix_nodejs_exist(win)) {
				...
				// Set command
				// [node][file]
				char* cmd = (char*)_webinix_malloc(16 + _webinix_strlen(full_path));
...
static const char* _webinix_generate_js_bridge(_webinix_window_t* win) {
	...
	// Non-authorized connection
		...
		printf("[Core]\t\t_webinix_generate_js_bridge() -> Non-authorized connection.\n");
#endif
...
// Generate the full Webinix Bridge
	...
	int c = sprintf(
	    js,
	    "%s\n document.addEventListener(\"DOMContentLoaded\",function(){ globalThis.webinix = new WebuiBridge({ "
	    "secure: %s, token: %" PRIu32 ", port: %zu, winNum: %zu, bindList: %s, log: %s, ",
	    webinix_javascript_bridge,
#ifdef WEBUI_TLS
	    "true",
#else
	    "false",
...
static bool _webinix_browser_create_new_profile(_webinix_window_t* win, size_t browser) {
	...
	if (browser == Chrome) {
...
#endif
		if (!win->custom_profile)
			sprintf(win->profile_path, "%s%s.Webinix%sWebinixFirefoxProfile", temp, webinix_sep, webinix_sep);
		...
		if (!_webinix_folder_exist(win->profile_path) ||
			...
			// prefs.js
			FILE* file;
			sprintf(buf, "%s%sprefs.js", win->profile_path, webinix_sep);
			file = fopen(buf, "a");
			if (file == NULL)
				return false;
			...
			// userChrome.css
			sprintf(buf, "\"%s%schrome%s\"", win->profile_path, webinix_sep, webinix_sep);
			if (!_webinix_folder_exist(buf)) {

				sprintf(buf, "mkdir \"%s%schrome%s\"", win->profile_path, webinix_sep, webinix_sep);
				_webinix_cmd_sync(win, buf, false); // Create directory
			}
			sprintf(buf, "%s%schrome%suserChrome.css", win->profile_path, webinix_sep, webinix_sep);
			file = fopen(buf, "a");
			if (file == NULL)
				return false;
#ifdef _WIN32
...
static bool _webinix_tls_generate_self_signed_cert(char* root_cert, char* root_key, char* ssl_cert, char* ssl_key) {
	...
	X509_NAME_add_entry_by_txt(root_name, "OU", MBSTRING_ASC, "Webinix", -1, -1, 0); // Organizational Unit
	...
	X509_NAME_add_entry_by_txt(root_name, "ST", MBSTRING_ASC, "Webinix", -1, -1, 0); // State
	X509_NAME_add_entry_by_txt(root_name, "L", MBSTRING_ASC, "Webinix", -1, -1, 0);  // Locality
    ...
    X509_NAME_add_entry_by_txt(name, "O", MBSTRING_ASC, "Webinix", -1, -1, 0); // Organization
    X509_NAME_add_entry_by_txt(name, "OU", MBSTRING_ASC, "Webinix", -1, -1, 0); // Organizational Unit
    ...
    X509_NAME_add_entry_by_txt(name, "ST", MBSTRING_ASC, "Webinix", -1, -1, 0); // State
    X509_NAME_add_entry_by_txt(name, "L", MBSTRING_ASC, "Webinix", -1, -1, 0); // Locality
...
static bool _webinix_show_window(_webinix_window_t* win, const char* content, bool is_embedded_html, size_t browser) {
	...
	if (_webinix_is_empty(_webinix_core.ssl_cert) || _webinix_is_empty(_webinix_core.ssl_key)) {
...
#ifdef WEBUI_LOG
		printf("[Core]\t\t_webinix_show_window() -> Generating self-signed TLS certificate...\n");
#endif
		...
		if (!_webinix_tls_generate_self_signed_cert(root_cert, root_key, ssl_cert, ssl_key)) {
			...
			char err_buf[1024];
			ERR_error_string_n(err, err_buf, sizeof(err_buf));
			printf("[Core]\t\t_webinix_show_window() -> Generating self-signed TLS certificate failed:\n%s\n", err_buf);
#endif
...
#ifdef WEBUI_LOG
		printf("[Core]\t\t_webinix_show_window() -> Self-signed SSL/TLS Certificate:\nRoot:\n");
...
static void _webinix_window_event(
	...
	if (!_webinix_core.exit_now && win->has_events) {
...
/*
    static char* _webinix_inject_bridge(_webinix_window_t* win, const char *user_html) {

        #ifdef WEBUI_LOG
            printf("[Core]\t\t_webinix_inject_bridge()...\n");
        #endif

        // Check if already exist
        if (strstr(user_html, "\"webinix.js\"") ||
            strstr(user_html, "\"/webinix.js\"") ||
        ...
        if (!head_pos) {
            ...
            size_t len = _webinix_strlen(user_html) + 128;
            char* buffer = (char*) _webinix_malloc(len);
            sprintf(buffer,
                "<html> <script type=\"application/javascript\" src=\"webinix.js\"></script>\n %s",
                user_html
            );
            return buffer;
        }
        ...
        // the Webinix-Bridge is too long to be inlined directely
        // so instead we will just use <script 'webinix.js'>.

        // Calculate the position right after <head>
        int offset = (int)(head_pos - user_html) + 6;

        // Calculate the length for the new string
        const char* script = "<script type=\"application/javascript\" src=\"webinix.js\"></script> \n";
        size_t new_len = strlen(user_html) + strlen(script) + 1;

        // Allocate memory for the new string
        char* buffer = (char*) _webinix_malloc(new_len);
...
static int _webinix_http_handler(struct mg_connection* conn, void* _win) {
	...
	if (_webinix_core.exit_now || win == NULL)
	...
	if (strcmp(ri->request_method, "GET") == 0) {
		...
		if (strcmp(url, "/webinix.js") == 0) {
...
#ifdef WEBUI_LOG
					printf("[Core]\t\t_webinix_http_handler() -> Embedded Index HTML Already Handled "
					       "(403)\n");
#endif
...
#ifdef WEBUI_LOG
					printf("[Core]\t\t_webinix_http_handler() -> Index local file Already Handled "
					       "(403)\n");
#endif
...
#ifdef WEBUI_LOG
					printf("[Core]\t\t_webinix_http_handler() -> Local Index File\n");
#endif

					// Set full path
					// [Path][Sep][File Name]
					char* index =
					    (char*)_webinix_malloc(_webinix_strlen(win->server_root_path) + 1 + 8);
			...
			//     #ifdef WEBUI_LOG
			//         printf("[Core]\t\t_webinix_http_handler() -> Local file .HTML Already Handled
			//         (403)\n");
			//     #endif
...
#ifdef WEBUI_LOG
				printf(
				    "[Core]\t\t_webinix_http_handler() -> Trying to interpret local file (Runtime = "
				    "%zu)\n",
				    win->runtime
				);
#endif
...
static int _webinix_ws_data_handler(struct mg_connection* conn, int opcode, char* data, size_t datasize, void* _win) {
	...
	if (_webinix_core.exit_now || datasize < WEBUI_PROTOCOL_SIZE)
		return 1; // OK
...
static void _webinix_ws_close_handler(const struct mg_connection* conn, void* _win) {
	...
	// Dereference
	_webinix_window_t* win = _webinix_dereference_win_ptr(_win);
	if (_webinix_core.exit_now || win == NULL || !win->connected)
		return;
...
static WEBUI_THREAD_SERVER_START {
...
#ifdef WEBUI_TLS
	ws_callbacks.init_ssl = _webinix_tls_initialization;
#endif
	...
	if (http_ctx && ws_ctx) {
		...
		mg_set_websocket_handler(
		    ws_ctx, "/_webinix_ws_connect", _webinix_ws_connect_handler, _webinix_ws_ready_handler,
			...
			while (!stop) {
				...
				if (!win->connected) {
					...
					while (!stop) {
						...
						if (!win->connected && !_webinix_core.exit_now) {
...
#ifdef WEBUI_LOG
			printf("[Core]\t\t_webinix_receive()... > Multi packet received more data than expected (%zu + %zu > %zu).\n", multi_receive, len, multi_expect);
#endif
			multi_packet = false;
			multi_expect = 0;
			multi_receive = 0;
...
#ifdef WEBUI_LOG
				printf("[Core]\t\t_webinix_receive()... > Multi packet started, Expecting %zu bytes...\n", expect_len);
#endif
...
#ifdef _WIN32
	HANDLE thread = CreateThread(NULL, 0, _webinix_receive_thread, (void*)arg, 0, NULL);
	if (thread != NULL)
		CloseHandle(thread);
#else
	pthread_t thread;
	pthread_create(&thread, NULL, &_webinix_receive_thread, (void*)arg);
	pthread_detach(thread);
#endif
}
...
static WEBUI_THREAD_RECEIVE {
	...
	if (!_webinix_core.exit_now) {
...
#ifdef WEBUI_LOG
		printf("[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Start...\n", recvNum);
#endif
		...
		if (event_type == WEBUI_WS_DATA) {
			...
			printf(
			    "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Packet Size : %zu bytes\n", recvNum,
			    packet_len
			);
			...
			printf("]\n");
			printf(
			    "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Packet Token: 0x%08X (%" PRIu32 ")\n",
			    recvNum, packet_token, packet_token
			);
			printf(
			    "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Packet ID: 0x%04X (%u)\n", recvNum,
			    packet_id, packet_id
			);
			printf("[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Packet Data: [", recvNum);
			//_webinix_print_ascii(&packet[WEBUI_PROTOCOL_DATA], (packet_len - WEBUI_PROTOCOL_SIZE));
			printf("]\n");
#endif
				...
				if (!_webinix_core.exit_now) { // Check if previous event called exit()
					...
					if ((unsigned char)packet[WEBUI_PROTOCOL_CMD] == WEBUI_CMD_CLICK) {
...
#ifdef WEBUI_LOG
						printf(
						    "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> WEBUI_CMD_CLICK "
						    "\n",
						    recvNum
						);
						printf(
						    "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Element size: "
						    "%zu bytes \n",
						    recvNum, element_len
						);
						printf(
						    "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Element : [%s] "
						    "\n",
						    recvNum, element
						);
#endif
...
#ifdef WEBUI_LOG
								printf(
								    "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> "
								    "WEBUI_CMD_JS \n",
								    recvNum
								);
								printf(
						...
						if (win->has_events) {
							...
							printf(
							    "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> URL "
							    "size: %zu bytes \n",
							    recvNum, url_len
							);
							printf(
						...
						printf(
						    "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Call ID: [%u] "
						    "\n",
						    recvNum, packet_id
						);
						printf(
...
// WebSocket/Civetweb did not send all the data as expected.
#ifdef WEBUI_LOG
							printf(
							    "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> No "
						...
						printf(
						    "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Unknown command "
						    "[0x%02x]\n",
					...
					printf(
					    "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Window disconnected.\n",
					    recvNum
					);
				}
#endif
			...
			printf(
			    "[Core]\t\t[Thread %zu] _webinix_receive_thread() -> UNKNOWN EVENT TYPE (%zu)\n", recvNum,
			    event_type
			);
		}
#endif
...
#ifdef WEBUI_LOG
		printf("[Core]\t\t[Thread %zu] _webinix_receive_thread() -> Finished.\n", recvNum);
#endif
	}
