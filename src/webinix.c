...
void webinix_set_icon(void* window, const char* icon, const char* type) {
...
bool webinix_show(void* window, const char* content) {
    ...
    unsigned int browser = _webinix_find_the_best_browser(win);
...
void _webinix_interface_bind_handler(webinix_event_t* e) {
    ...
    if(cb_index > 0 && _webinix_core.cb_interface[cb_index] != NULL) {
        ...
        #endif
        _webinix_core.cb_interface[cb_index](e->window, e->type, e->element, e->data, (char*)&e->response);
    }
...
unsigned int webinix_interface_bind(void* window, const char* element, void (*func)(void*, unsigned int, char*, char*, char*)) {
...
void webinix_interface_set_response(char* ptr, const char* response) {
        ...
        printf("[User] webinix_interface_set_response() -> Pointer @ 0x%p \n", ptr);
    ...
    *_ptr = buf;
    ...
    #ifdef WEBUI_LOG
        printf("[User] webinix_interface_set_response() -> Internal buffer @ 0x%p \n", (*_ptr));
...
bool _webinix_browser_start_firefox(_webinix_window_t* win, const char* address) {
    ...
    char full[1024];
    sprintf(full, "%s -P Webinix -purgecaches -new-window %s", win->browser_path, address);
...
unsigned int _webinix_find_the_best_browser(_webinix_window_t* win) {
    ...
    // To save memory, let's search if a web browser is already running
        ...
        else if(_webinix_is_process_running("google-chrome") && _webinix_browser_exist(win, Chrome)) return Chrome;
        else if(_webinix_is_process_running("google-chrome-stable") && _webinix_browser_exist(win, Chrome)) return Chrome;
        else if(_webinix_is_process_running("microsoft-edge-stable") && _webinix_browser_exist(win, Edge)) return Edge;
        else if(_webinix_is_process_running("microsoft-edge-dev") && _webinix_browser_exist(win, Edge)) return Edge;
        ...
        else if(_webinix_is_process_running("brave-browser") && _webinix_browser_exist(win, Brave)) return Brave;
        else if(_webinix_is_process_running("firefox") && _webinix_browser_exist(win, Firefox)) return Firefox;
        else if(_webinix_is_process_running("yandex-browser") && _webinix_browser_exist(win, Yandex)) return Yandex;
        else if(_webinix_is_process_running("chromium-browser") && _webinix_browser_exist(win, Chromium)) return Chromium;
    #else
        // macOS
...
static void _webinix_window_event(_webinix_window_t* win, char* webinix_internal_id, char* element, void* data, unsigned int data_len, int event_type) {
    ...
    _webinix_cb_t* arg = (_webinix_cb_t*) _webinix_malloc(sizeof(_webinix_cb_t));
    arg->win = win;
...
WEBUI_CB
{
    _webinix_cb_t* arg = (_webinix_cb_t*) _arg;
    ...
    if(arg->element_name != NULL && !_webinix_is_empty(arg->element_name)) {
