...
static bool _webinix_browser_create_new_profile(_webinix_window_t * win, size_t browser) {
    ...
    if (browser == Chrome) {
        ...
        if (!_webinix_folder_exist(win->profile_path) ||
            ...
            // prefs.js
            FILE * file;
            WEBUI_SPF_DYN(buf, sizeof(buf), "%s%sprefs.js", win->profile_path, webinix_sep);
            ...
            // userChrome.css
            WEBUI_SPF_DYN(buf, sizeof(buf), "\"%s%schrome%s\"", win->profile_path, webinix_sep, webinix_sep);
            if (!_webinix_folder_exist(buf)) {

                WEBUI_SPF_DYN(buf, sizeof(buf), "mkdir \"%s%schrome%s\"", win->profile_path, webinix_sep, webinix_sep);
                _webinix_cmd_sync(win, buf, false); // Create directory
            }
            WEBUI_SPF_DYN(buf, sizeof(buf), "%s%schrome%suserChrome.css", win->profile_path, webinix_sep, webinix_sep);
            WEBUI_FOPEN(file, buf, "a");
            if (file == NULL)
                return false;
            #ifdef _WIN32
        ...
        if (strcmp(url, "/webinix.js") == 0) {
            ...
            if (win->is_embedded_html) {
                ...
                if (win->html_handled) {
                    ...
                    // Index.ts
                    WEBUI_SPF_DYN(index, bf_len, "%s%sindex.ts", win->server_root_path, webinix_sep);
                    ...
                    // Index.js
                    WEBUI_SPF_DYN(index, bf_len, "%s%sindex.js", win->server_root_path, webinix_sep);
                    if (_webinix_file_exist(index)) {
