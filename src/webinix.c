...
#define WEBUI_CMD_WIN_DRAG   0xF4    // Command: Drag window (Webinix `-webkit-app-region: drag;` custom implementation)
#define WEBUI_CMD_WIN_RESIZED 0xF3   // Command: Update window coordinates for Webinix `-webkit-app-region: drag;` custom implementation
...
// WebView
    ...
    typedef int (*g_timeout_add_func)(int, int (*function)(void*), void*); // Webinix exit loop
...
#else
    extern bool _webinix_macos_wv_new(int index, bool frameless, bool resizable);
    extern void _webinix_macos_wv_new_thread_safe(int index, bool frameless, bool resizable);
...
typedef struct _webinix_window_t {
    ...
    bool proxy_set;
    char *proxy_server;
    webinix_mutex_t mutex_win_exit_now;
    bool win_exit_now;
    // WebView
    bool allow_webview;
    bool allow_browser;
...
typedef struct _webinix_core_t {
    ...
    webinix_mutex_t mutex_app_exit_now;
...
static bool _webinix_mutex_app_is_exit_now(int update);
static bool _webinix_mutex_win_is_exit_now(_webinix_window_t* win, int update);
...
static void _webinix_send_all(_webinix_window_t* win, uint16_t id, unsigned char cmd, const char* data, size_t len);
...
static int _webinix_external_file_handler(_webinix_window_t* win, struct mg_connection* client, size_t client_id);
...
static bool _webinix_load_gtk_and_webkit();
#else
// macOS
...
void webinix_run_client(webinix_event_t* e, const char* script) {
    ...
    // Dereference
    if (_webinix_mutex_app_is_exit_now(WEBUI_MUTEX_GET_STATUS) || _webinix.wins[e->window] == NULL)
        return;
...
size_t webinix_new_window_id(size_t num) {
...
void webinix_destroy(size_t window) {
    ...
    _webinix_mutex_destroy(&win->mutex_win_exit_now);
...
bool webinix_show_wv(size_t window, const char* content) {
    ...
    // Show for all connected clients
    return _webinix_show(win, NULL, content, Webview);
}
...
const char* webinix_get_mime_type(const char* file) {

    #ifdef WEBUI_LOG
    printf("[User] webinix_get_mime_type([%s])\n", file);
    #endif
...
void webinix_set_config(webinix_config option, bool status) {
    ...
    switch (option) {
        ...
        case asynchronous_response:
            _webinix.config.asynchronous_response = status;
            break;
...
void webinix_set_minimum_size(size_t window, unsigned int width, unsigned int height) {
    ...
    #ifdef WEBUI_LOG
    printf("[User] webinix_set_minimum_size(%zu, %u, %u)\n", window, width, height);
    #endif
...
void webinix_set_center(size_t window) {
    #ifdef WEBUI_LOG
    printf("[User] webinix_set_center(%zu)\n", window);
    #endif
    ...
    if (!libgtk || !libwebkit) {
        if (_webinix_load_gtk_and_webkit()) {
    ...
    #ifdef WEBUI_LOG
    printf("[User] webinix_set_center() -> Screen dimensions: width=%u, height=%u\n", screen_width, screen_height);
    #endif
    ...
    if (screen_width >= 100 && screen_height >= 100) {
        if (win->width >= WEBUI_MIN_WIDTH && win->height >= WEBUI_MIN_HEIGHT) {
            ...
            #ifdef WEBUI_LOG
            printf("[User] webinix_set_center() -> Window dimensions: width=%u, height=%u\n", win->width, win->height);
            printf("[User] webinix_set_center() -> Calculated center positions: x=%u, y=%u\n", center_x, center_y);
            #endif
            // Set window center position
            webinix_set_position(window, center_x, center_y);
        }
    }
}
            ...
            if (_webinix_mutex_is_connected(_webinix.wins[i], WEBUI_MUTEX_GET_STATUS)) {
                ...
                if (!_webinix.wins[i]->webView) {
                    ...
                    if (_webinix.wins[i]->webView) {
                        ...
                        _webinix_webview_update(_webinix.wins[i]);
                    }        
                }
            }
        }
...
bool webinix_set_root_folder(size_t window, const char* path) {
    ...
    if (win->server_running || 
        _webinix_is_empty(path) || 
        (_webinix_strlen(path) > WEBUI_MAX_PATH) ||
        !_webinix_folder_exist(path)) {
...
static void _webinix_interface_bind_handler_all(webinix_event_t* e) {

    #ifdef WEBUI_LOG
    printf("[Core]\t\t_webinix_interface_bind_handler_all()\n");
    #endif
    ...
    if (win->has_all_events) {
        ...
        if (exist && win->cb_interface[events_cb_index] != NULL) {
            ...
            #ifdef WEBUI_LOG
            printf(
                "[Core]\t\t_webinix_interface_bind_handler_all() -> User all-events callback @ 0x%p\n",
                win->cb_interface[events_cb_index]
            );
            printf("[Core]\t\t_webinix_interface_bind_handler_all() -> User all-events e->event_type [%zu]\n", e->event_type);
            printf("[Core]\t\t_webinix_interface_bind_handler_all() -> User all-events e->element [%s]\n", e->element);
            printf("[Core]\t\t_webinix_interface_bind_handler_all() -> User all-events e->event_number %zu\n", e->event_number);
            printf("[Core]\t\t_webinix_interface_bind_handler_all() -> User all-events e->bind_id %zu\n", e->bind_id);
            #endif
            // Call all-events cb
            #ifdef WEBUI_LOG
            printf("[Core]\t\t_webinix_interface_bind_handler_all() -> Calling user all-events callback\n[Call]\n");
            #endif
...
static void _webinix_interface_bind_handler(webinix_event_t* e) {
    ...
    if (!_webinix_mutex_app_is_exit_now(WEBUI_MUTEX_GET_STATUS) && !_webinix_is_empty(e->element)) {
        size_t cb_index = 0;
...
size_t webinix_interface_bind(size_t window, const char* element, void(*func)(size_t, size_t, char* , size_t, size_t)) {
    ...
    size_t cb_index = 0;
    if (_webinix_is_empty(element)) {
        cb_index = webinix_bind(window, "", _webinix_interface_bind_handler_all);
    } else {
        cb_index = webinix_bind(window, element, _webinix_interface_bind_handler);
    }
...
void webinix_interface_set_response_file_handler(size_t window, const void* response, int length) {
    ...
    printf("[User] webinix_interface_set_response_file_handler() -> Response %d bytes\n", length);
    #endif
...
bool webinix_interface_show_client(size_t window, size_t event_number, const char* content) {
    ...
    #ifdef WEBUI_LOG
    printf("[User] webinix_interface_show_client()\n");
    #endif
    ...
    return webinix_show_client(&e, content);
}

void webinix_interface_close_client(size_t window, size_t event_number) {

    #ifdef WEBUI_LOG
    printf("[User] webinix_interface_close_client()\n");
    #endif
    ...
    webinix_close_client(&e);
}

void webinix_interface_send_raw_client(size_t window, size_t event_number, const char* function, const void* raw, size_t size) {

    #ifdef WEBUI_LOG
    printf("[User] webinix_interface_send_raw_client()\n");
    #endif
    ...
    webinix_send_raw_client(&e, function, raw, size);
}

void webinix_interface_navigate_client(size_t window, size_t event_number, const char* url) {

    #ifdef WEBUI_LOG
    printf("[User] webinix_interface_navigate_client()\n");
    #endif
...
void webinix_interface_run_client(size_t window, size_t event_number, const char* script) {
    ...
    webinix_run_client(&e, script);
}

bool webinix_interface_script_client(size_t window, size_t event_number, const char* script, size_t timeout, char* buffer, size_t buffer_length) {

    #ifdef WEBUI_LOG
    printf("[User] webinix_interface_script_client()\n");
    #endif
...
static bool _webinix_ptr_exist(void * ptr) {
    ...
    for (size_t i = 0; i < _webinix.ptr_last_pos; i++) {
...
static void _webinix_ptr_add(void * ptr, size_t size) {
    ...
    if (!_webinix_ptr_exist(ptr)) {
        size_t i = 0;
        for (; i < _webinix.ptr_last_pos; i++) {
            if (_webinix.ptr_list[i] == NULL) {
                // Pointer found
                break;
            }
        }
        if (i == _webinix.ptr_last_pos) {
            // Pointer not found
            i = _webinix.ptr_last_pos++;
            if (_webinix.ptr_last_pos >= (WEBUI_MAX_IDS * 2)) {
                _webinix.ptr_last_pos = ((WEBUI_MAX_IDS * 2) - 1);
                #ifdef WEBUI_LOG
                printf("[Core]\t\t_webinix_ptr_add(0x%p) -> ERROR: Maximum pointer capacity reached.\n",
                ptr);
                #endif
            }
        }
...
static void _webinix_free_mem(void * ptr) {
    ...
    for (int i = _webinix.ptr_last_pos; i >= 0;i--) {
        if (_webinix.ptr_list[i] == NULL) {
            _webinix.ptr_last_pos = i;
            break;
        }
    }
...
static void _webinix_free_all_mem(void) {
    ...
    for (size_t i = 0; i < _webinix.ptr_last_pos; i++) {
        ...
        if (ptr != NULL) {
            #ifdef WEBUI_LOG
            printf("[Core]\t\t_webinix_free_all_mem() -> Pointer #%zu freed (%zu + 1 bytes) 0x%p\n",
            i, _webinix.ptr_size[i], ptr);
            #endif
            free(ptr);
            _webinix.ptr_size[i] = 0;
            _webinix.ptr_list[i] = NULL;
...
static int _webinix_external_file_handler(_webinix_window_t* win, struct mg_connection* client, size_t client_id) {
    ...
    #ifdef WEBUI_LOG
    printf("[Core]\t\t_webinix_external_file_handler()\n");
    #endif
    ...
    if (win->files_handler != NULL || win->files_handler_window != NULL) {
        ...
        #ifdef WEBUI_LOG
        printf("[Core]\t\t_webinix_external_file_handler() -> Path [%s]\n", url);
        ...
        if (callback_resp != NULL) {
            ...
            #ifdef WEBUI_LOG
            printf("[Core]\t\t_webinix_external_file_handler() -> Custom files handler found (%zu bytes)\n",
                length
            );
            #endif
            ...
            _webinix_print_ascii((const char*)callback_resp, length);
            printf("\n------------------------------------\n");
            #endif
            ...
            _webinix_free_mem((void*)callback_resp);

            http_status_code = 200;
        }
        else {
            #ifdef WEBUI_LOG
            printf("[Core]\t\t_webinix_external_file_handler() -> Custom files handler failed\n");
            #endif
        }
...
static int _webinix_serve_file(_webinix_window_t* win, struct mg_connection* client, size_t client_id) {
    ...
    #ifdef WEBUI_LOG
    printf("[Core]\t\t_webinix_serve_file() -> Looking for file locally\n");
    #endif
...
static bool _webinix_bun_exist(_webinix_window_t* win) {

    #ifdef WEBUI_LOG
    printf("[Core]\t\t_webinix_bun_exist()\n");
    #endif
    ...
    if (_webinix_cmd_sync(win, "bun -v", false) == 0) {
...
static int _webinix_interpret_file(_webinix_window_t* win, struct mg_connection* client, char* index, size_t client_id) {
    ...
    if (strcmp(extension, "ts") == 0 || strcmp(extension, "js") == 0) {
        ...
        if (win->runtime == Deno) {
            ...
            // Use Bun
            if (_webinix_bun_exist(win)) {

                // Set command
                // [bun][file][query]
                size_t bf_len = (32 + _webinix_strlen(full_path) + _webinix_strlen(query));
                char* cmd = (char*)_webinix_malloc(bf_len);
...
static bool _webinix_mutex_is_single_client_token_valid(_webinix_window_t* win, int update) {
...
static bool _webinix_mutex_is_multi_client_token_valid(_webinix_window_t* win, int update, int index) {
...
static bool _webinix_mutex_app_is_exit_now(int update) {
    ...
    bool status = false;
    _webinix_mutex_lock(&_webinix.mutex_app_exit_now);
    if (update == WEBUI_MUTEX_SET_TRUE) _webinix.app_exit_now = true;
    else if (update == WEBUI_MUTEX_SET_FALSE) _webinix.app_exit_now = false;
    status = _webinix.app_exit_now;
    _webinix_mutex_unlock(&_webinix.mutex_app_exit_now);
    return status;
}
...
static bool _webinix_mutex_win_is_exit_now(_webinix_window_t* win, int update) {
    ...
    bool status = false;
    _webinix_mutex_lock(&win->mutex_win_exit_now);
    ...
    _webinix_mutex_unlock(&win->mutex_win_exit_now);
    return status;
}
...
static void _webinix_webview_update(_webinix_window_t* win) {
    ...
    #elif __linux__
    // Linux - GTK WebView
    _webinix_condition_signal(&win->condition_webview_update);
    #else
    // macOS - WKWebView
    _webinix_condition_signal(&win->condition_webview_update);
    #endif
...
static bool _webinix_browser_create_new_profile(_webinix_window_t* win, size_t browser) {
    ...
    if (browser == Chrome) {
        ...
        // Google Chrome
        if (!win->custom_profile)
            WEBUI_SN_PRINTF_DYN(win->profile_path, WEBUI_MAX_PATH, "%s%s.Webinix%sWebinixChromeProfile", temp, os_sep, os_sep);
        return true;
    } else if (browser == Edge) {

        // Edge
        if (!win->custom_profile)
            WEBUI_SN_PRINTF_DYN(win->profile_path, WEBUI_MAX_PATH, "%s%s.Webinix%sWebinixEdgeProfile", temp, os_sep, os_sep);
        return true;
    } else if (browser == Epic) {

        // Epic
        if (!win->custom_profile)
            WEBUI_SN_PRINTF_DYN(win->profile_path, WEBUI_MAX_PATH, "%s%s.Webinix%sWebinixEpicProfile", temp, os_sep, os_sep);
        return true;
    } else if (browser == Vivaldi) {

        // Vivaldi
        if (!win->custom_profile)
            WEBUI_SN_PRINTF_DYN(win->profile_path, WEBUI_MAX_PATH, "%s%s.Webinix%sWebinixVivaldiProfile", temp, os_sep, os_sep);
        return true;
    } else if (browser == Brave) {

        // Brave
        if (!win->custom_profile)
            WEBUI_SN_PRINTF_DYN(win->profile_path, WEBUI_MAX_PATH, "%s%s.Webinix%sWebinixBraveProfile", temp, os_sep, os_sep);
        return true;
    } else if (browser == Yandex) {

        // Yandex
        if (!win->custom_profile)
            WEBUI_SN_PRINTF_DYN(win->profile_path, WEBUI_MAX_PATH, "%s%s.Webinix%sWebinixYandexProfile", temp, os_sep, os_sep);
        return true;
    } else if (browser == Chromium) {

        // Chromium
        if (!win->custom_profile)
            WEBUI_SN_PRINTF_DYN(win->profile_path, WEBUI_MAX_PATH, "%s%s.Webinix%sWebinixChromiumProfile", temp, os_sep, os_sep);
        return true;
        ...
        // ~/snap/firefox/common/.mozilla/firefox/WebinixFirefoxProfile
        ...
        if (!win->custom_profile){
            ...
            if(!win->disable_browser_high_contrast) {
                ff = "WebinixFirefoxProfile-NoHC";
            }
            // Generate Firefox profile path
            if (WEBUI_SN_PRINTF_DYN(win->profile_path, WEBUI_MAX_PATH, "%s%s.Webinix%s%s",
                temp, os_sep, os_sep, ff) >= WEBUI_MAX_PATH) {
                // Generated path is too big
                #ifdef WEBUI_LOG
                printf(
                    "[Core]\t\t_webinix_browser_create_new_profile(%zu) -> "
...
static bool _webinix_custom_browser_exist(_webinix_window_t* win, size_t browser) {

    #ifdef WEBUI_LOG
    printf("[Core]\t\t_webinix_custom_browser_exist([%zu])\n", browser);
    #endif
    ...
    if (_webinix_folder_exist(_webinix.custom_browser_folder_path)) {
        char full_path[WEBUI_MAX_PATH] = {0};
        WEBUI_SN_PRINTF_STATIC(
            full_path, WEBUI_MAX_PATH,
            "%s\\%s", _webinix.custom_browser_folder_path, executable
        );
        if (_webinix_file_exist(full_path)) {
            // Browser Found
    ...
    if (_webinix_folder_exist(_webinix.custom_browser_folder_path)) {
        ...
        if (_webinix_folder_exist(full_app_path)) {
    ...
    if (_webinix_folder_exist(_webinix.custom_browser_folder_path)) {
        ...
        for (int i = 0; i < MAX_BROWSER_FILES && executables[i] != NULL; i++) {
            WEBUI_SN_PRINTF_STATIC(
                ...
                "%s/%s", _webinix.custom_browser_folder_path, executables[i]
            );
            if (_webinix_file_exist(full_path)) {
                // Browser Found
                if(win) WEBUI_SN_PRINTF_DYN(win->browser_path, WEBUI_MAX_PATH, "\"%s\"", full_path);
...
static bool _webinix_browser_exist(_webinix_window_t* win, size_t browser) {
    ...
    if (browser == Chrome) {
        ...
        if (_webinix_custom_browser_exist(win, browser)) {
            ChromeExist = true;
            return true;
        }
...
static void _webinix_clean(void) {
    ...
    _webinix_mutex_destroy(&_webinix.mutex_app_exit_now);
    ...
    _webinix_mutex_destroy(&_webinix.mutex_token);
...
static bool _webinix_show(_webinix_window_t* win, struct mg_connection* client, const char* content, size_t browser) {
    ...
    // Folder
        ...
        #endif
        // Set root folder
        if (!webinix_set_root_folder(win->num, content_cpy)) {
            #ifdef WEBUI_LOG
...
static bool _webinix_tls_generate_self_signed_cert(char* root_cert, char* root_key, char* ssl_cert, char* ssl_key) {
    ...
    X509_NAME_add_entry_by_txt(root_name, "C", MBSTRING_ASC, (const unsigned char *)"CA", -1, -1,
        0); // Country
    X509_NAME_add_entry_by_txt(root_name, "O", MBSTRING_ASC, (const unsigned char *)"Webinix Root Authority", -1, -1, 0); // Organization
    X509_NAME_add_entry_by_txt(root_name, "OU", MBSTRING_ASC, (const unsigned char *)"Webinix", -1, -1,
    ...
    X509_NAME_add_entry_by_txt(root_name, "ST", MBSTRING_ASC, (const unsigned char *)"Webinix", -1, -1,
        0); // State
    X509_NAME_add_entry_by_txt(root_name, "L", MBSTRING_ASC, (const unsigned char *)"Webinix", -1, -1,
        0); // Locality
    ...
    X509_NAME_add_entry_by_txt(name, "C", MBSTRING_ASC, (const unsigned char *)"CA", -1, -1,
        0); // Country
    X509_NAME_add_entry_by_txt(name, "O", MBSTRING_ASC, (const unsigned char *)"Webinix", -1, -1,
        0); // Organization
    X509_NAME_add_entry_by_txt(name, "OU", MBSTRING_ASC, (const unsigned char *)"Webinix", -1, -1,
        0); // Organizational Unit
    ...
    X509_NAME_add_entry_by_txt(name, "ST", MBSTRING_ASC, (const unsigned char *)"Webinix", -1, -1,
        0); // State
    X509_NAME_add_entry_by_txt(name, "L", MBSTRING_ASC, (const unsigned char *)"Webinix", -1, -1,
        0); // Locality
...
static bool _webinix_show_window(_webinix_window_t* win, struct mg_connection* client, const char* content, int type, size_t browser) {
    ...
    // Initialization
    _webinix_mutex_win_is_exit_now(win, WEBUI_MUTEX_SET_FALSE);
    if (win->html != NULL)
        _webinix_free_mem((void*)win->html);
    if (win->url != NULL)
        _webinix_free_mem((void*)win->url);
    ...
    if (!_webinix_mutex_is_connected(win, WEBUI_MUTEX_GET_STATUS)) {
        ...
        if (browser != NoBrowser) {
            if (!runWebView && !runBrowser) {
                ...
                _webinix_mutex_win_is_exit_now(win, WEBUI_MUTEX_SET_TRUE);
                _webinix_free_mem((void*)win->html);
                _webinix_free_mem((void*)win->url);
    ...
    if ((browser != NoBrowser) && _webinix.config.show_wait_connection) {
        ...
        #ifdef WEBUI_LOG
        printf("[Core]\t\t_webinix_show_window() -> Waiting for window connection & token validation\n");
        #endif
...
static void _webinix_window_event(
    ...
    // Check for all events-bind functions
    if (!_webinix_mutex_app_is_exit_now(WEBUI_MUTEX_GET_STATUS) && win->has_all_events) {
        size_t events_cb_index = 0;
        bool exist = _webinix_get_cb_index(win, "", &events_cb_index);
...
static void _webinix_init(void) {
    ...
    _webinix_mutex_init(&_webinix.mutex_app_exit_now);
    ...
    _webinix_mutex_init(&_webinix.mutex_token);
        ...
        for (size_t i = 0; i < WEBUI_MAX_IDS; i++) {
            if (win->html_elements[i] != NULL) {
                if (strcmp(win->html_elements[i], element) == 0) {
                    #ifdef WEBUI_LOG_VERBOSE
                    printf("[Core]\t\t_webinix_get_cb_index() -> Found at %zu\n", i);
                    #endif
                    _webinix_mutex_unlock(&_webinix.mutex_bridge);
                    *id = i;
                    return true;
...
static void _webinix_http_send_header(
    ...
    printf("[Core]\t\t_webinix_http_send_header() -> mime_type: [%s]\n", mime_type);
    printf("[Core]\t\t_webinix_http_send_header() -> body_len: [%zu]\n", body_len);
    printf("[Core]\t\t_webinix_http_send_header() -> cache: [%d]\n", cache);
    #endif
    ...
    if (_webinix.config.use_cookies) {
        ...
        if (!client_found) {
            ...
            if (_webinix_client_cookies_save(win, new_auth_cookies, &new_client_id)) {
                set_cookies = true;
                _webinix.cookies_single_set[win->num] = true;
                #ifdef WEBUI_LOG
    ...
    if (set_cookies) {
        ...
        to_send = WEBUI_SN_PRINTF_STATIC(buffer, sizeof(buffer),
            "HTTP/1.1 200 OK\r\n"
            "Set-Cookie: webinix_auth=%s; Path=/; HttpOnly; SameSite=Strict\r\n"
...
//     }
//     // Single
//     _webinix.cookies_single_set[win->num] = false;
// }
...
static int _webinix_http_handler(struct mg_connection* client, void * _win) {
    ...
    if (_webinix_mutex_app_is_exit_now(WEBUI_MUTEX_GET_STATUS) || win == NULL) {
    ...
    if (strcmp(ri->request_method, "GET") == 0) {
        ...
        if (!_webinix.config.multi_client) {
            if (!client_found) {
                if ((_webinix.cookies_single_set[win->num])) {
                    #ifdef WEBUI_LOG
            ...
            } else _webinix.cookies_single_set[win->num] = true;
        }
        ...
        if (strcmp(url, "/webinix.js") == 0) {
            ...
            // nothing to do now.
            #ifdef WEBUI_LOG
            printf("[Core]\t\t_webinix_http_handler() -> Handled by custom external file handler\n");
            #endif
        }
            ...
            if (_webinix_file_exist(folder_path)) {
            ...
            else if (_webinix_folder_exist(folder_path)) {
                ...
                #ifdef WEBUI_LOG
                printf("[Core]\t\t_webinix_http_handler() -> Not found\n");
                #endif
...
static int _webinix_ws_data_handler(struct mg_connection* client, int opcode, char* data, size_t datasize, void * _win) {
    ...
    if (_webinix_mutex_app_is_exit_now(WEBUI_MUTEX_GET_STATUS) || datasize < WEBUI_PROTOCOL_SIZE)
        return 1; // OK
...
static void _webinix_ws_close_handler(const struct mg_connection* client, void * _win) {
    ...
    // Dereference
    _webinix_window_t* win = _webinix_dereference_win_ptr(_win);
    if (_webinix_mutex_app_is_exit_now(WEBUI_MUTEX_GET_STATUS) || win == NULL || !_webinix_mutex_is_connected(win, WEBUI_MUTEX_GET_STATUS))
        return;
...
static WEBUI_THREAD_SERVER_START {
    ...
    if (http_ctx) {
        ...
        if (_webinix.startup_timeout > 0) {
            ...
            printf("[Core]\t\t_webinix_server_thread([%zu]) -> Timeout is %zu seconds\n", 
                win->num, _webinix.startup_timeout);
            #endif
            ...
            while(!stop) {
                ...
                if (!_webinix_mutex_is_connected(win, WEBUI_MUTEX_GET_STATUS)) {
                    ...
                    for (;;) {
                        ...
                        if (_webinix_mutex_app_is_exit_now(WEBUI_MUTEX_GET_STATUS) || _webinix_mutex_win_is_exit_now(win, WEBUI_MUTEX_GET_STATUS)) {
                            stop = true;
                            break;
                        }
                    ...
                    if (_webinix.config.folder_monitor && !monitor_created) {
                        ...
                        #ifdef _WIN32
                        monitor_thread = CreateThread(NULL, 0, _webinix_folder_monitor_thread, (void*)win, 0, NULL);
    ...
    if (_webinix.config.folder_monitor && monitor_created) {
        #ifdef WEBUI_LOG
        printf("[Core]\t\t_webinix_server_thread([%zu]) -> Killing folder monitor thread\n", win->num);
        #endif
        #ifdef _WIN32
...
static bool _webinix_connection_save(_webinix_window_t* win, struct mg_connection* client, size_t* connection_id) {
    #ifdef WEBUI_LOG
    printf("[Core]\t\t_webinix_connection_save([%zu])\n", win->num);
    #endif
    ...
    for (size_t i = 0; i < WEBUI_MAX_IDS; i++) {
        if (_webinix.clients[i] == NULL) {
            #ifdef WEBUI_LOG
            printf("[Core]\t\t_webinix_connection_save() -> Registering client #%zu \n", i);
            #endif
            // Save
    ...
    // List is full
    #ifdef WEBUI_LOG
    printf("[Core]\t\t_webinix_connection_save() -> Clients list is full\n");
    #endif
    _webinix_mutex_unlock(&_webinix.mutex_client);
    return false;
}
...
static void _webinix_connection_remove(_webinix_window_t* win, struct mg_connection* client) {
    #ifdef WEBUI_LOG
    printf("[Core]\t\t_webinix_connection_remove([%zu])\n", win->num);
    #endif
    ...
    for (size_t i = 0; i < WEBUI_MAX_IDS; i++) {
        if (_webinix.clients[i] == client) {
            #ifdef WEBUI_LOG
            printf("[Core]\t\t_webinix_connection_remove() -> Removing client #%zu \n", i);
            #endif
            // Reset Token
            if (!_webinix.config.multi_client) {
    ...
    // Client not found
    #ifdef WEBUI_LOG
    printf("[Core]\t\t_webinix_connection_remove() -> Client not found\n");
    #endif
    _webinix_mutex_unlock(&_webinix.mutex_client);
    mg_close_connection(client);
}
...
static bool _webinix_connection_get_id(_webinix_window_t* win, struct mg_connection* client, size_t* connection_id) {
    #ifdef WEBUI_LOG
    printf("[Core]\t\t_webinix_connection_get_id([%zu], [%p])\n", win->num, client);
    #endif
    ...
    // Client not found
    #ifdef WEBUI_LOG
    printf("[Core]\t\t_webinix_connection_get_id() -> Client not found\n");
    #endif
    _webinix_mutex_unlock(&_webinix.mutex_client);
    return false;
}
    ...
    if (!_webinix_mutex_app_is_exit_now(WEBUI_MUTEX_GET_STATUS)) {
        ...
        if (event_type == WEBUI_WS_DATA) {
            ...
            if ((len >= WEBUI_PROTOCOL_SIZE) &&
                ...
                if (!_webinix_mutex_app_is_exit_now(WEBUI_MUTEX_GET_STATUS)) { // Check if previous event called exit()

                    if ((unsigned char)packet[WEBUI_PROTOCOL_CMD] == WEBUI_CMD_WIN_DRAG) {

                        // Drag Window Event (Webinix `-webkit-app-region: drag;` custom implementation)
                        ...
                        #ifdef WEBUI_LOG
                        printf("[Core]\t\t_webinix_ws_process(%zu) -> Drag Window: X = %d (0x%08X), Y = %d (0x%08X)\n", recvNum, X, x, Y, y);
                        #endif

                        // Move the dragged window
                        webinix_set_position(win->num, X, Y);
                        ...
                        if (data_size_expected == data_size_recv) {
                            ...
                            e.cookies = (char*)_webinix_get_cookies_full(client); // Full cookies
...
// -- WebView -------------------------
    ...
    static WEBUI_THREAD_WEBVIEW {
        ...
        if (!_webinix.webview_cacheFolder) {
            ...
            WEBUI_SN_PRINTF_DYN(_webinix.webview_cacheFolder, WEBUI_MAX_PATH, 
                "%s%s.Webinix%sWebinixWebViewCache_%"PRIu32, temp, os_sep, os_sep,
            ...
            while (true) {
                ...
                if (PeekMessageW(&msg, NULL, 0, 0, PM_REMOVE) > 0) {
                    ...
                    if (_webinix_mutex_is_webview_update(win, WEBUI_MUTEX_GET_STATUS)) {
                        ...
                        // At this moment, there is no Win32 or Webinix messages.
    ...
    static void _webinix_wv_create(_webinix_window_t* win) {
        ...
        // Webinix Exit Event for GTK
        g_timeout_add((1 * 1000), _webinix_wv_exit_schedule, NULL);
    }
    ...
    static int _webinix_wv_exit_schedule(void* arg) {
        ...
        #ifdef WEBUI_LOG
        // printf("[Core]\t\t_webinix_wv_exit_schedule()\n");
        #endif

        if (_webinix_mutex_app_is_exit_now(WEBUI_MUTEX_GET_STATUS)) {
            if (_webinix.is_gtk_main_run) {
                #ifdef WEBUI_LOG
                printf("[Core]\t\t_webinix_wv_exit_schedule() -> Quit GTK Main Loop...\n");
                #endif
                gtk_main_quit();
            }
            return 0;
    ...
    static bool _webinix_load_gtk_and_webkit() {
        if (!libgtk || !libwebkit) {
            ...
            for (size_t i = 0; i < (sizeof(gtk_libs) / sizeof(gtk_libs[0]));++i) {
                ...
                if (libgtk) {
                    #ifdef WEBUI_LOG
                    printf("[Core]\t\t_webinix_load_gtk_and_webkit() -> GTK loaded [%s]\n", 
                    gtk_libs[i]);
                    #endif
                    break;
                }
            ...
            for (size_t i = 0; i < (sizeof(webkit_libs) / sizeof(webkit_libs[0]));++i) {
                ...
                if (libwebkit) {
                    #ifdef WEBUI_LOG
                    printf("[Core]\t\t_webinix_load_gtk_and_webkit() -> WebKit loaded [%s]\n", 
                    webkit_libs[i]);
                    #endif
                    break;
                }
            ...
            {
                #ifdef WEBUI_LOG
                printf("[Core]\t\t_webinix_load_gtk_and_webkit() -> GTK symbol addresses failed\n");
                #endif
                _webinix_wv_free();
                return false;
            }
            ...
            if (!webkit_web_view_new || !webkit_web_view_load_uri || !webkit_web_view_get_title) {
                #ifdef WEBUI_LOG
                printf("[Core]\t\t_webinix_load_gtk_and_webkit() -> WebKit symbol addresses failed\n");
                #endif
                _webinix_wv_free();
                return false;
            }
    ...
    static bool _webinix_wv_show(_webinix_window_t* win, char* url) {
        ...
        #ifdef WEBUI_DYNAMIC
        #ifdef WEBUI_LOG
        printf("[Core]\t\t_webinix_wv_show() -> Webinix dynamic version does not support Linux WebView\n");
        #endif
        return false;
        #endif
        ...
        if (_webinix.wins[index] != NULL) {
            ...
            if (_webinix.wins[index]->webView) {
                ...
                if (_webinix.wins[index]->webView) {
                    ...
                    _webinix_webview_update(_webinix.wins[index]);
                }
            }
        }
    }
    ...
    static bool _webinix_wv_show(_webinix_window_t* win, char* url) {
        ...
        if (!_webinix.is_wkwebview_main_run) {
            if (_webinix_macos_wv_new(win->num, win->frameless, win->resizable)) {
                if (!_webinix.is_webview) {
            ...
            _webinix_macos_wv_new_thread_safe(win->num, win->frameless, win->resizable);
            _webinix_sleep(250);
        }
...
static WEBUI_THREAD_MONITOR {
    ...
    // Loop trough all windows
    for (size_t i = 1; i < WEBUI_MAX_IDS; i++) {
        if ((_webinix.wins[i] != NULL) && (_webinix.wins[i] != win) && (i < win->num)) {
            WEBUI_THREAD_RETURN
        }
    }

    const char* js = "location.reload();";
    size_t js_len = _webinix_strlen(js);
        ...
        char buffer[1024];
        DWORD bytesReturned;
        while ((!_webinix_mutex_app_is_exit_now(WEBUI_MUTEX_GET_STATUS)) && (win->server_running)) {
        ...
        #endif
        char buffer[1024];
        while (!_webinix_mutex_app_is_exit_now(WEBUI_MUTEX_GET_STATUS)) {
