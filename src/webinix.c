...
static bool _webinix_tls_generate_self_signed_cert(char* ssl_cert, char* ssl_key);
#endif
...
#ifdef _WIN32
			// Windows
			_webinix_remove_firefox_ini_profile(
			    "%APPDATA%\\Mozilla\\Firefox\\profiles.ini", win->profile_name
			);
			_webinix_delete_folder(win->profile_path);
#elif __linux__
			// Linux
			_webinix_remove_firefox_ini_profile("~/.mozilla/firefox/profiles.ini", win->profile_name);
			_webinix_delete_folder(win->profile_path);
#else
			// macOS
			_webinix_remove_firefox_ini_profile(
...
bool webinix_set_tls_certificate(const char* certificate_pem, const char* private_key_pem) {
	...
	if (!_webinix_is_empty(certificate_pem) && !_webinix_is_empty(private_key_pem)) {
		...
		printf("[User] webinix_set_tls_certificate() -> SSL/TLS Private Key:\n");
		printf(
		    "- - -[Key]- - - - - - - - - - -\n%s\n- - - - - - - - - - - - - - - - -\n",
...
static bool _webinix_browser_create_new_profile(_webinix_window_t* win, size_t browser) {
	...
	if (browser == Chrome) {
		...
		if (!_webinix_folder_exist(win->profile_path) ||
			...
			_webinix_remove_firefox_ini_profile(path_ini, win->profile_name);
			_webinix_delete_folder(win->profile_path);
...
static bool _webinix_tls_generate_self_signed_cert(char* ssl_cert, char* ssl_key) {
	...
	X509_NAME_add_entry_by_txt(name, "CN", MBSTRING_ASC, "Webinix", -1, -1, 0); // Common Name
...
static bool _webinix_show_window(_webinix_window_t* win, const char* content, bool is_embedded_html, size_t browser) {
	...
	if (_webinix_is_empty(_webinix_core.ssl_cert) || _webinix_is_empty(_webinix_core.ssl_key)) {
		...
		if (!_webinix_tls_generate_self_signed_cert(ssl_cert, ssl_key)) {
#ifdef WEBUI_LOG
		...
		printf("[Core]\t\t_webinix_show_window() -> Self-signed SSL/TLS Key:\n");
		printf(
		    "- - -[Key]- - - - - - - - - -\n%s\n- - - - - - - - - - - - - - - -\n",
		    (const char*)_webinix_core.ssl_key
		);
