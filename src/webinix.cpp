...
namespace webinix {
	...
	const std::string javascriptbridge = (R"V0G0N(
		...
		var _webinix_do_win_close = true;
...
namespace webinix{
	...
	const std::string html_served = "<html><head></head>"
								 ...
								 "Sorry, This Webinix window is already served.</body></html>";
	struct event{
	...
	class _window {
        ...
        size_t bind(std::string key_id, void(*function_ref)(webinix::event e)) const;
...
namespace webinix{
	...
	size_t getkey(const std::string& id){
		...
		for (size_t i = 0; i < webinix::key_v.size(); i++){
	...
	size_t _window::bind(std::string key_id, void(*function_ref)(webinix::event e)) const{

		key_id = this->settings.number_s + "/" + key_id;

		size_t id = webinix::getkey(key_id);
