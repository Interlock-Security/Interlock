			...
			if ('WebSocket' in window){
				...
				_webinix_ws.onmessage = function (evt){
					...
					reader.addEventListener("loadend", function(e){
						...
						if(buffer8.length < _webinix_minimum_data)
							return;
...
namespace webinix{
	...
	// void start_webserver(const char * ip, unsigned short port, const std::string * html, webinix::_window * ui);
	// void start_websocket(const char * ip, unsigned short port, webinix::_window * ui);
	...
	class _window {
        ...
        struct {
			...
			std::array<void(*)(webinix::event e), 1> key_action_all;
        ...
        unsigned short bind(std::string key_id, void(*function_ref)(webinix::event e)) const;
        void bind_all(void(*function_ref)(webinix::event e));
...
namespace BoostWebServer{
	...
	class http_connection : public std::enable_shared_from_this<http_connection>{
		...
		public:
		//const std::string * html_main = &webinix::html_def;
		webinix::_window *p_ui = nullptr;
		...
		process_request()
		{
			...
			switch(request_.method())
			{
				...
				response_.set(http::field::server, "Webinix");
				create_response();
				break;
		...
		create_response()
		{
			...
			if(request_.target().substr(0, 13) == "/webinixcore.js"){
				...
				if(!this->p_ui->settings.webserver_allow_multi && this->p_ui->settings.webserver_served){
					...
					response_.set(http::field::content_type, "text/html; charset=utf-8");
					beast::ostream(response_.body()) << webinix::html_served;
				...
				if(this->p_ui->settings.icon){
					...
					response_.set(http::field::content_type, webinix::def_icon_type);
					beast::ostream(response_.body()) << webinix::def_icon;
				}
			}
			else {
	...
	class http_loop {
		...
		public:
		//const std::string * html_def = &webinix::html_def;
		webinix::_window *p_ui;
...
namespace BoostWebSocket{
	...
	void BoostWebSocket::session::run(webinix::_window * ui){
	...
	class listener : public std::enable_shared_from_this<listener>
	{
		...
		void fail(beast::error_code ec, char const* what){
			...
			std::cerr << "[!] Webinix -> Websocket -> Listener failed on port " + this->p_ui->settings.websocket_port + " -> " << what << ": " << ec.message() << "\n";
		...
		void
		run(bool * WindowWasConnected, webinix::_window * ui)
		{
	...
	void listener::on_accept(beast::error_code ec, tcp::socket socket){
			...
			if(ec){
				...
				*this->p_WindowWasConnected = true;
				...
				webinix::session_actions[this->p_ui->settings.number] = p_session;
			}
...
namespace webinix{
	...
	namespace browser{
		...
		bool browser_exist(unsigned short browser){
			...
			if(browser == firefox){
				...
				// Firefox
					...
					std::string fullpath32 = programs_folder32 + webinix::sep + "Mozilla Firefox\\firefox.exe";
					std::string fullpath64 = programs_folder64 + webinix::sep + "Mozilla Firefox\\firefox.exe";
				...
				// Chrome
					...
					std::string fullpath32 = programs_folder32 + webinix::sep + "Google\\Chrome\\Application\\chrome.exe";
					std::string fullpath64 = programs_folder64 + webinix::sep + "Google\\Chrome\\Application\\chrome.exe";
				...
				// Edge
					...
					std::string fullpath32 = programs_folder32 + webinix::sep + "Microsoft\\Edge\\Application\\msedge.exe";
					std::string fullpath64 = programs_folder64 + webinix::sep + "Microsoft\\Edge\\Application\\msedge.exe";
		...
		std::string get_temp_path(unsigned short browser){
			...
			std::cerr << "[!] Webinix -> Browser -> Unknow browser ID\n";
			webinix::exit();
			return "";
		}
		...
		bool create_profile_folder(unsigned short browser){
			...
			if(browser == chrome){
				...
				frofile_path = temp + webinix::sep + "WebinixChromeProfile";
				return true;
			}
			
			if(browser == edge){

				frofile_path = temp + webinix::sep + "WebinixEdgeProfile";
				return true;
			}
			...
			if(!folder_exist(temp + webinix::sep + profile_name)){
				...
				// prefs.js
				std::fstream prefs(temp + webinix::sep + profile_name + webinix::sep + "prefs.js", std::ios::out | std::ios::app);
					if(prefs.fail())
						return false;
				...
				// userChrome.css
				boost::filesystem::create_directory(temp + webinix::sep + profile_name + webinix::sep + "chrome" + webinix::sep);
				std::fstream userChrome(temp + webinix::sep + profile_name + webinix::sep + "chrome" + webinix::sep + "userChrome.css", std::ios::out | std::ios::app);
				if(prefs.fail())
					return false;
				...
				userChrome.close();
				frofile_path = temp + webinix::sep + profile_name;
			}
	...
	void ini(){
		...
		webinix::key_v.emplace_back("");
		webinix::port_v.clear();

		webinix::use_timeout = true;
		webinix::startup_timeout = 10; // 10 seconds
	}
	...
	signed short getkey(const std::string& id){
		...
		for (unsigned short i = 0; i < static_cast<unsigned short>(webinix::key_v.size()); i++){

			if (webinix::key_v[i] == id){
	...
	unsigned short setkey(const std::string& id){
		...
		webinix::key_v.push_back(id);
		return static_cast<unsigned short>(webinix::key_v.size()) - 1;
	}
	...
	void set_timeout_sec(unsigned short s){
		...
		if(s < 1)
			...
			webinix::use_timeout = true;
			webinix::startup_timeout = s;
		}
	}

	void waitforstartup(){

		if(webinix::connected_swindow > 0)
			return;

		// Wait for a specific time
		for(unsigned short n = 0; n <= (webinix::startup_timeout * 2); n++){
	...
	void loop(){
		...
		if(webinix::use_timeout){
			...
			if(webinix::waitfor_swindow){

				// Wait one time while user 
				// browser is starting!
				webinix::waitforstartup();

				while(webinix::connected_swindow > 0){
	...
	void FreePort(unsigned short p){
		...
		webinix::port_v.erase(std::remove(webinix::port_v.begin(), webinix::port_v.end(), p), webinix::port_v.end());
	}
	...
	void FreeWindow(){
		...
		if(webinix::connected_swindow < 1)
			webinix::waitfor_swindow = false;
	}
	...
	unsigned short getport(){
		...
		webinix::port_v.push_back(p);
		return p;
	}

	unsigned short getwin(){
		
		return webinix::winlast++;
	}

	void start_webserver(const char * ip, unsigned short port, webinix::_window * ui){
		...
		try {
			...
			webinix::msgbox(L"Error.\nWebinix failed to start http web server.");
			std::cerr << "[!] Webinix -> Webserver -> Start failed on port " + ui->settings.websocket_port + " -> " << e.what() << "\n";
		}

		// Clean
		ui->settings.webserver_running = false;
		ui->settings.webserver_served = false;
		webinix::FreePort(port);
	}
	...
	void start_websocket(const char * ip, unsigned short port, webinix::_window * ui){
		...
		if(WindowWasConnected)
			webinix::FreeWindow();
		webinix::session_actions[ui->settings.number] = nullptr;
		webinix::FreePort(port);
	...
	_window::_window(){
		...
		this->settings.number = webinix::getwin();
	...
	unsigned short get_nat_id(){
		...
		for(unsigned short i = 0; i <= 1024; i++){
			...
			if(std::find(webinix::nat_id_v.begin(), webinix::nat_id_v.end(), id) != webinix::nat_id_v.end()){
	...
	void free_nat_id(unsigned short id){
		...
		webinix::nat_data[id] = nullptr;
		webinix::nat_id_v.erase(std::remove(webinix::nat_id_v.begin(), webinix::nat_id_v.end(), id), webinix::nat_id_v.end());
	}
	...
	void _window::send(std::vector<std::uint8_t> &packets_v) const{
		...
		if(webinix::session_actions[this->settings.number] && webinix::session_actions[this->settings.number] != nullptr)
			webinix::session_actions[this->settings.number]->send(packets_v); // TODO: make send(const packets_v)
	...
	std::string _window::run(std::string js, unsigned short seconds) const{
		...
		// Initializing pipe
		std::string buf_pipe;
		const unsigned short id = webinix::get_nat_id();
		webinix::nat_data_status[id] = false;
		webinix::nat_data[id] = &buf_pipe;
	...
	void _window::receive(std::vector<std::uint8_t> &packets_v){
		...
		if (type == TYPE_EVENT){
			...
			if(webinix::nat_data[id] == nullptr)
				return;
	...
	void _window::websocket_session_clean(){
		...
		webinix::session_actions[this->settings.number] = nullptr;
	}

	void _window::event(const std::string& id, const std::string& element){

		int key_id = webinix::getkey(id);
		...
		if(key_id >= 0 && webinix::key_actions[key_id]){

			e.element_id = key_id;

			// webinix::key_actions[key_id](e);
			std::thread t(webinix::key_actions[key_id], e);
    		t.detach();
		}
	}

	void _window::bind_all(void(*function_ref)(webinix::event e)){

		this->settings.key_action_all[0] = function_ref;
		this->settings.is_bind_all = true;
	}

	unsigned short _window::bind(std::string key_id, void(*function_ref)(webinix::event e)) const{

		key_id = this->settings.number_s + "/" + key_id;

		signed short id = webinix::getkey(key_id);
		
		if(id >= 0){

			// Replace a reference
			webinix::key_actions[id] = function_ref;
		}
		else {
	...
	bool _window::window_show(const std::string * html, unsigned short browser){
		...
		// Initializing
		unsigned short port_ws				= webinix::getport();
		this->settings.html					= html;
		this->settings.webserver_served		= false;
		webinix::waitfor_swindow				= true;

		auto _start_websocket = [&](){

			// -- Web Socket ----------------------------
			std::thread StartWebSocket_job(	&webinix::start_websocket,	// pointer
											"127.0.0.1",				// IP
											port_ws,					// port
											this);						// obj
		...
		if(!this->window_is_running()){
			...
			unsigned short port_web = webinix::getport();

			// -- Web Server ----------------------------
			std::thread StartWebServer_job(	&webinix::start_webserver,	// pointer
											"127.0.0.1",				// IP
											port_web,					// port
											this);						// obj
