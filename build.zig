...
fn build_examples_12(b: *Build, optimize: OptimizeMode, target: Build.ResolvedTarget, webinix_lib: *Compile) void {
...
fn build_examples_11(b: *Build, optimize: OptimizeMode, target: CrossTarget, webinix_lib: *Compile) void {
    var lazy_path = Build.LazyPath{
        .path = "examples/C",
    };
...
fn build_webinix_12(b: *Build, optimize: OptimizeMode, target: Build.ResolvedTarget, is_static: bool, enable_tls: bool) *Compile {
    const name = "webinix";
    const webinix = if (is_static) b.addStaticLibrary(.{ .name = name, .target = target, .optimize = optimize }) else b.addSharedLibrary(.{ .name = name, .target = target, .optimize = optimize });
...
fn build_webinix_11(b: *Build, optimize: OptimizeMode, target: CrossTarget, is_static: bool, enable_tls: bool) *Compile {
    const name = "webinix";
    const webinix = if (is_static) b.addStaticLibrary(.{ .name = name, .target = target, .optimize = optimize }) else b.addSharedLibrary(.{ .name = name, .target = target, .optimize = optimize });
...
fn build_11(b: *Build) void {
    ...
    const webinix = build_webinix_11(b, optimize, target, isStatic, enableTLS);

    webinix.installHeader("include/webinix.h", "webinix.h");

    build_examples_11(b, optimize, target, webinix);
