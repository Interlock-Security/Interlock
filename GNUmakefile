...
WEBUI_BUILD_FLAGS := -o webinix.o -I"$(MAKEFILE_DIR)/include/" -c "$(MAKEFILE_DIR)/src/webinix.c" -I"$(WEBUI_TLS_INCLUDE)" $(TLS_CFLAG) -w
...
ifeq ($(shell uname),Darwin)
	@cd "$(BUILD_DIR)/debug" \
	&& echo "Build Webinix Objective-C WKWebKit ($(CC) $(TARGET)debug)..." \
	&& $(CC) $(TARGET) $(WKWEBKIT_BUILD_FLAGS) -g -DWEBUI_LOG
endif
#	Static with Debug info
	@cd "$(BUILD_DIR)/debug" \
	&& echo "Build Webinix library ($(CC) $(TARGET)debug static)..." \
	&& $(CC) $(TARGET) $(CIVETWEB_BUILD_FLAGS) $(CIVETWEB_DEFINE_FLAGS) -g \
	&& $(CC) $(TARGET) $(WEBUI_BUILD_FLAGS) -g -DWEBUI_LOG \
	&& $(LLVM_OPT)ar rc $(LIB_STATIC_OUT) webinix.o civetweb.o $(WEBKIT_OBJ) \
	&& $(LLVM_OPT)ranlib $(LIB_STATIC_OUT)
#	Dynamic with Debug info
	@cd "$(BUILD_DIR)/debug" \
	&& echo "Build Webinix library ($(CC) $(TARGET)debug dynamic)..." \
	&& $(CC) $(TARGET) $(CIVETWEB_BUILD_FLAGS) $(CIVETWEB_DEFINE_FLAGS) -g -fPIC \
	&& $(CC) $(TARGET) $(WEBUI_BUILD_FLAGS) -g -fPIC -DWEBUI_LOG \
	&& $(CC) $(TARGET) -shared -o $(LIB_DYN_OUT) webinix.o civetweb.o $(WEBKIT_OBJ) -g -L"$(WEBUI_TLS_LIB)" $(TLS_LDFLAG_DYNAMIC) $(LWS2_OPT) $(WKWEBKIT_LINK_FLAGS)
ifeq ($(PLATFORM),windows)
...
ifeq ($(shell uname),Darwin)
	@cd "$(BUILD_DIR)" \
	&& echo "Build Webinix Objective-C WKWebKit ($(CC) $(TARGET)release)..." \
	&& $(CC) $(TARGET) $(WKWEBKIT_BUILD_FLAGS) -Os
endif
#	Static Release
	@cd "$(BUILD_DIR)" \
	&& echo "Build Webinix library ($(CC) $(TARGET)release static)..." \
...
#	Dynamic Release
	@cd "$(BUILD_DIR)" \
	&& echo "Build Webinix library ($(CC) $(TARGET)release dynamic)..." \
	&& $(CC) $(TARGET) $(CIVETWEB_BUILD_FLAGS) $(CIVETWEB_DEFINE_FLAGS) -Os -fPIC \
	&& $(CC) $(TARGET) $(WEBUI_BUILD_FLAGS) -O3 -fPIC \
	&& $(CC) $(TARGET) -shared -o $(LIB_DYN_OUT) webinix.o civetweb.o $(WEBKIT_OBJ) -L"$(WEBUI_TLS_LIB)" $(TLS_LDFLAG_DYNAMIC) $(LWS2_OPT) $(WKWEBKIT_LINK_FLAGS)
#	Clean
ifeq ($(PLATFORM),windows)
