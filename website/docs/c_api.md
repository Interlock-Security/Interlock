...
This document includes all Webinix C APIs.
...
char* response; // No need to be used, its internally used by webinix_return_xxx()
```
...
Webinix waits a couple of seconds (_Default is 10 seconds_) to let the web browser start and connect. You can control this behaviour by using `webinix_set_timeout()`.
...
After the window is loaded, the URL is not valid anymore for safety. Webinix will show an error if someone else tries to access the URL. To allow multi-user access to the same URL, you can use `webinix_multi_access()`.
...
webinix_show(win, "my_file.ts");

// Nodejs
webinix_set_runtime(my_window, Nodejs);
webinix_show(win, "my_file.js");
```
